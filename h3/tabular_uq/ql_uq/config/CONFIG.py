...
Key differences from pes_base/config/CONFIG.py
-----------------------------------------
- MAX_SEVERITY = 9  (same as pes_base) → Q-table shape 31 × 11 × 10 × 11
- SEED = 42         (same as pes_base) → deterministic Q-Learning training
...
# ==================== OUTPUT & LOGGING ====================
OUTPUT_FILE_PREFIX = 'PES_QL_'    # Prefix for all output filenames
...
# When True, the resource allocation chosen via argmax(Q) is scaled down by a
# risk-controlled confidence factor instead of being executed as-is.
# Domain is discrete: action ∈ {0, 1, ..., MAX_ALLOCATABLE_RESOURCES},
# with sequence budget AVAILABLE_RESOURCES_PER_SEQUENCE = 39, so the effective
# action is clamped to [0, min(resources_left, MAX_ALLOCATABLE_RESOURCES)].
# Penalty modes:
#   1: 'linear'             -> Linear scaling: 1 - gamma * (1 - C)
#   2: 'boltzmann_option_a' -> Severity-attenuated Boltzmann factor: exp(-gamma * (1 - C) / ((S_t + 1) * (C + eps)))
#   3: 'boltzmann_option_b' -> Pure uncertainty Boltzmann factor: exp(-gamma * (1 - C) / (C + eps))
CONFIDENCE_ALLOCATION_PENALTY = True
PENALTY_MODE = {
    1: 'linear',
    2: 'boltzmann_option_a',
    3: 'boltzmann_option_b',
}[1]  # <-- SELECT: Change index (1/2/3) to choose penalty mode

CONFIDENCE_RISK_FACTOR = 0.1106009227695729  # Risk factor / gamma (γ) constant tuned by Optuna
BOLTZMANN_EPSILON = 1e-4        # Thermal stability constant (ε) for Boltzmann modes
