...
##  Imports internos    ##
##########################
from .. import ANSI, INPUTS_PATH, NUM_SEQUENCES, SEQ_LENGTHS_FILE, VERBOSE, CONFIDENCE_ALLOCATION_PENALTY
from ..ext.pandemic import apply_confidence_risk_penalty
...
def rl_agent_meta_cognitive(options, resources_left, response_timeout):
  ...
  # available resources, trial, severity
    ...
    # Scale the chosen allocation down by confidence, when enabled: this is
    # the only place in the interactive path where the penalty applies,
    # since ``response`` here is already used as the real game action.
    if CONFIDENCE_ALLOCATION_PENALTY:
        response = apply_confidence_risk_penalty(
            int(response),
            float(confidence),
            int(resources_left) if resources_left is not None else 0,
        )

    # Ensure response never exceeds available resources
    available_resources = int(resources_left) if resources_left is not None else 0
    response = int(numpy.clip(response, 0, available_resources))
...
def provide_rl_agent_response(
    ...
    Notes
    -----
    - Requires Q-table pre-training via: python3 -m tabular.pes_ql.ext.train_rl
    ...
    if not os.path.isfile(q_file):
        raise FileNotFoundError(
            ...
            f"Please train the RL-Agent first by running: python3 -m tabular.pes_ql.ext.train_rl\n"
        )
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Error: {str(e)}\n"
            f"Files may be corrupted. Please retrain by running: python3 -m tabular.pes_ql.ext.train_rl\n"
        ) from e
    ...
    resp, confidence, rt_hold, rt_release = rl_agent_meta_cognitive(
        Q[resources_idx, city_idx, sever_idx], resources_left, RESPONSE_TIMEOUT)
