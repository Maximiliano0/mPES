...
pes_ql - Pandemic Experiment Scenario
 ...
 Bayesian Optimization of Q-Learning and confidence-risk hyperparameters using Optuna.

Optimizes the Q-Learning hyperparameters plus the confidence-risk penalty
parameters supported by the h3 environment.
Objective: maximize mean normalised performance over a fixed 32-sequence subset.
...
Usage:
    python3 -m tabular_uq.ql_uq.ext.optimize_rl [n_trials] [--resume YYYY-MM-DD]
...
Search space:
    penalty_mode     ∈ {'linear', 'boltzmann_option_a', 'boltzmann_option_b'}
    learning_rate    ∈ [0.05, 0.40]       (log scale)
    discount_factor  ∈ [0.85, 0.999]
    epsilon_initial  ∈ [0.50, 1.00]
    epsilon_min      ∈ [0.01, 0.15]
    num_episodes     ∈ [400000, 800000]   (step=50000)
    risk_factor      ∈ [0.00, 1.00] (linear) or [0.00, 3.00] (boltzmann)
...
def notify(*_args, **_kwargs):
    """Provide a no-op notification hook when no notifier is configured."""
    return None
...
# A fixed subset keeps trial scores comparable while reducing evaluation I/O.
_OPTIMIZATION_EVAL_SEQUENCES = 32
...
def objective(trial: optuna.Trial) -> float:
    """Optuna objective for the confidence ``risk_factor`` search.

    Samples all seven effective Q-Learning and h3 penalty parameters.
    The Q-table is trained with the per-trial seed ``SEED + trial.number + 1``.
    The greedy policy is evaluated with infeasible-action masking on a fixed 32-
    sequence subset, ``NaN``/``Inf`` are sanitised to 0 and the score is clipped
    to ``[0, 1]`` so the TPE surrogate is not poisoned, then
    ``_best_artifacts`` is updated (and persisted to ``_opt_dir`` without
    ``pickle``) when the score improves.

    Parameters
    ----------
    trial : optuna.Trial
        The Optuna trial driving the ``risk_factor`` sampling.

    Returns
    -------
    float
        Mean normalised performance over the 32 evaluation sequences,
        clipped to ``[0, 1]``.
    """
    ...
    penalty_mode = trial.suggest_categorical(
        'penalty_mode', ['linear', 'boltzmann_option_a', 'boltzmann_option_b']
    )
    max_risk = 3.0 if penalty_mode.startswith('boltzmann') else 1.0
    risk_factor = trial.suggest_float('risk_factor', 0.0, max_risk)

    # --- Train with this trial's complete hyperparameter configuration ---
    env = Pandemic()
    ...
    rewards, Q, _ = QLearning(
        env, learning_rate, discount_factor, epsilon_initial, epsilon_min,
        num_episodes,
        seed=SEED + int(trial.number) + 1,
    ...
    def qf(_env, state, _seqid):
        ...
        action = int(numpy.argmax(options))
        _, confidence, _, _ = rl_agent_meta_cognitive(
            Q[s0, s1, s2].copy(),
            int(state[0]),
            10_000,
        )
        return apply_confidence_risk_penalty(
            action,
            float(confidence),
            int(state[0]),
            severity=float(state[2]),
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        _, perfs, _ = run_experiment(
            ...
            qf,
            False,
            _trials_per_sequence[:_OPTIMIZATION_EVAL_SEQUENCES],
            _sevs[:_OPTIMIZATION_EVAL_SEQUENCES],
            NumberOfIterations=_OPTIMIZATION_EVAL_SEQUENCES,
        )
...
def _save_report(study, opt_dir, opt_date, best_Q, best_rewards):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 80 + "\n")
        f.write(
            f"{'#':>4s}  {'mean_perf':>10s}  {'lr':>10s}  {'gamma':>8s}  "
            f"{'eps0':>6s}  {'eps_min':>7s}  {'episodes':>8s}  "
            f"{'penalty_mode':>20s}  {'risk_factor':>11s}\n"
        )
        f.write("-" * 80 + "\n")
...
###################################
def main():
    """Run Bayesian optimisation of the h3 Q-Learning configuration.
    ...
    (seeded with ``SEED``), and finally writes the best Q-table,
    reward history, the complete best-trial hyperparameter sidecar,
    reproducibility fingerprint, optimisation
    report and convergence plots to ``inputs/<date>_BAYESIAN_OPT/``. The
    standard input paths (``q.npy``, ``rewards.npy``,
    ``best_params.json``) are mirrored automatically so ``train_rl.py``
    and ``__main__.py`` can consume them without manual copying.
    ...
    list_item("num_episodes     ∈ [400000, 800000]   (step=50000)")
    list_item("penalty_mode     ∈ {linear, boltzmann_option_a, boltzmann_option_b}")
    list_item("risk_factor      ∈ [0.00, 1.00] (linear) or [0.00, 3.00] (boltzmann)")
    print()
    ...
    def _progress_callback(study, trial):
        ...
        try:
            ...
            best_val = None
            best_str = "  n/a  "
        # For a PRUNED trial, Optuna stores the last *reported* intermediate
        # value (the raw average reward from trial.report()) as trial.value,
        # not the [0, 1]-scaled mean_perf objective. Printing that number as
        # "value=" would look like a normalised score while actually being on
        # a completely different scale, so show the trial state instead.
        ...
        print(
            ...
            f"mean_perf={value_str}  |  best={best_str}  |  "
        ...
        if done > 0 and done % 10 == 0:
            ...
            notify(
                ...
                f"Último trial: mean_perf={value_str}\n"
    ...
    # --- Best Q-table from the winning trial ---
    ...
    if _best_artifacts['Q'] is not None and _best_artifacts['value'] >= best.value:
        ...
        raise FileNotFoundError(
            "Best trial artifacts are missing. Resume requires "
            "_best_artifacts.npz and _best_artifacts.json."
        )

    list_item(f"Q-table shape: {best_Q.shape}")
    list_item(f"Best trial number (0-based): {best.number}")
    list_item(f"Training seed:               {SEED + best.number + 1}")
