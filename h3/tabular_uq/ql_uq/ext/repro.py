...
def find_reference_artifacts() -> tuple[str, str | None] | None:
    """Locate the best tuned params used by the h1 reference package.

    The h3 uncertainty package is designed to compare against the already-tuned
    h1 tabular pes_ql baseline, so it prefers the h1 best-params payload when it
    is available and only falls back to local h3 artifacts afterwards.

    Returns
    -------
    tuple of str or None
        ``(params_path, fingerprint_path)`` for the newest h1 reference run, or
        ``None`` if no such reference data exists.
    """
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..'))
    h1_inputs = os.path.join(repo_root, 'h1', 'tabular', 'pes_ql', 'inputs')
    if not os.path.isdir(h1_inputs):
        return None

    candidates: list[tuple[str, str, str]] = []
    for entry in os.listdir(h1_inputs):
        full = os.path.join(h1_inputs, entry)
    ...
    if not candidates:
        std_path = os.path.join(h1_inputs, 'best_params.json')
        if os.path.isfile(std_path):
            return std_path, None
        return None
