...
from .. import ANSI, INPUTS_PATH, CONFIDENCE_ALLOCATION_PENALTY
...
def main():
    ...
    if artifacts is None:
        raise FileNotFoundError(
            "No h3 Bayesian optimization JSON found. Run ext/optimize_rl.py first."
        )

    params_path, fp_path = artifacts
    params = load_params(params_path)
    info(f"Loaded best h3 optimization JSON: {os.path.basename(params_path)}")
    ...
    if fp_path and os.path.isfile(fp_path):
        ...
        info("Optimization fingerprint unavailable; using the JSON as the comparison baseline.")
    ...
    num_episodes    = int(hp['num_episodes'])
    risk_factor     = float(hp['risk_factor'])
    penalty_mode    = str(hp['penalty_mode'])
    # Accept both canonical (best_trial_number / trial_seed / mean_perf) and
    # legacy (trial_number / training_seed / expected_mean_perf) field names
    # so old sidecars keep working after the 2026-04-21 schema unification.
    # Guard against missing keys: legacy sidecars may omit either name; default to -1
    # so downstream messages flag the absence instead of crashing on ``int(None)``.
    trial_number    = int(params.get('best_trial_number', params.get('trial_number', -1)) or -1)
    training_seed   = int(params.get('trial_seed',
                          params.get('training_seed', SEED + trial_number + 1)) or 0)
    expected_perf   = params.get('mean_perf', params.get('expected_mean_perf'))
    ...
    list_item(f"penalty_mode     = {penalty_mode}")
    list_item(f"risk_factor      = {risk_factor}")
    ...
    if True:  # pylint: disable=using-constant-test
        ...
        def eval_qf(_env, state, _seqid):
            ...
            # Confidence — used for visualisation and, when
            # CONFIDENCE_ALLOCATION_PENALTY is enabled, to scale down the
            # allocation: floor(response * confidence).
            ...
            if CONFIDENCE_ALLOCATION_PENALTY and confidence >= 0:
                response = apply_confidence_risk_penalty(
                    int(response),
                    float(confidence),
                    int(state[0]) if state[0] is not None else 0,
                    severity=float(state[2]) if len(state) > 2 and state[2] is not None else 0.0,
