...
#     repo's <group>/<pkg>/inputs/ directories (so general.runner can find
#     the .keras / .npy / best_params.json files).
#  2. Mirrors general/results/ from Drive (resume support).
#  3. Runs python -m general.orchestrate (then aggregate / plot / report).
...
# ─── Run the sweep ───────────────────────────────────────────────────────
echo "════════════════════════════════════════════════════════════════════════"
echo "  Launching general.orchestrate"
echo "════════════════════════════════════════════════════════════════════════"
ORCH_ARGS=()
[[ -n "$PKG_FILTER" ]] && ORCH_ARGS+=( --pkg "$PKG_FILTER" )
python3 -m general.orchestrate "${ORCH_ARGS[@]}" 2>&1 | tee -a "$DRIVE_BENCH/orchestrate.log"

# ─── Aggregate + plot + report ───────────────────────────────────────────
echo "  → aggregate"
python3 -m general.aggregate    2>&1 | tee -a "$DRIVE_BENCH/aggregate.log"
echo "  → plot"
python3 -m general.plot_matrix  2>&1 | tee -a "$DRIVE_BENCH/plot.log"
echo "  → report"
python3 -m general.report       2>&1 | tee -a "$DRIVE_BENCH/report.log"
