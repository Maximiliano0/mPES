...
| `ml` | `pes_ens` | Ensemble (soft voting of pes_dqn + pes_rdqn + pes_trf; pes_a2c configured but disabled by default) |
...
- Adapt the package name to its full grouped path
  ...
  `ml.pes_ens`) based on where the user wants to place the script.
- Do NOT cross-reference between packages — each package is
  self-contained. The single intentional exception is `ml/pes_ens`,
