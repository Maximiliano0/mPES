...
- `$PACKAGE` — the target package.
  Valid values: `pes_ql`, `pes_dql`, `pes_dqn`, `pes_rdqn`, `pes_a2c`, `pes_trf`.
  Internally each resolves to its grouped path (`tabular/pes_*` or
  `ml/pes_*`) — see the package map below.
...
Each package has its own optimisation module, located inside its grouped
folder:

| Package | Group | Module | Alias(es) |
|---------|-------|--------|-----------|
| `pes_ql` | `tabular` | `tabular.pes_ql.ext.optimize_rl` | `bayesian`, `bay`, `1` |
| `pes_dql` | `tabular` | `tabular.pes_dql.ext.optimize_rl` | `dql`, `ql`, `2` |
| `pes_dqn` | `ml` | `ml.pes_dqn.ext.optimize_dqn` | `dqn`, `3` |
| `pes_rdqn` | `ml` | `ml.pes_rdqn.ext.optimize_rdqn` | `rdqn`, `7` |
| `pes_a2c` | `ml` | `ml.pes_a2c.ext.optimize_a2c` | `ac`, `a2c`, `actor-critic`, `4` |
| `pes_trf` | `ml` | `ml.pes_trf.ext.optimize_tr` | `transformer`, `tr`, `5` |

If the user provides an alias instead of the full package name, resolve it
using the table above. The launcher scripts in `utils/win/` and
`utils/linux/` already perform this resolution internally.
...
**Linux:**
```bash
./utils/linux/run_bayesian_opt.sh dqn 110
./utils/linux/run_bayesian_opt.sh ac 100
./utils/linux/run_bayesian_opt.sh bayesian 100 2026-02-12   # resume
```

**Windows (PowerShell):**
```powershell
.\utils\win\run_bayesian_opt.ps1 dqn 110
.\utils\win\run_bayesian_opt.ps1 ac 100
.\utils\win\run_bayesian_opt.ps1 bayesian 100 2026-02-12  # resume
```

These scripts handle environment activation, power settings and background
...
- `PKG` — short package name (e.g. `pes_dqn`).
- `GROUP` — group folder (`tabular` or `ml`).
- `OPT_MODULE` — full Python module path including the group prefix
  (e.g. `ml.pes_dqn.ext.optimize_dqn`).

### Step 2 — Prepare the log directory

```bash
LOG_DIR="${GROUP}/${PKG}/inputs"
mkdir -p "$LOG_DIR"
```
...
Same table as above. Derive `$PkgName`, `$Group` (`tabular` or `ml`),
and `$OptModule` (e.g. `ml.pes_dqn.ext.optimize_dqn`).

### Step 2 — Prepare the log directory

```powershell
$LogDir = Join-Path $PWD "$Group\$PkgName\inputs"
...
Use `utils/scripts/run_module.py` to redirect stdout/stderr at the Python level.
...
$RunModule = Join-Path $PWD 'utils\scripts\run_module.py'
