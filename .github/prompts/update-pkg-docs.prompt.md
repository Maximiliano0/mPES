...
- `ml/`      — `pes_dqn`, `pes_rdqn`, `pes_a2c`, `pes_trf`, `pes_ens`.
...
When invoking this prompt, specify the **target package** name (short or
grouped form), or `all` to process every package in the workspace.
Examples:

```
@update-pkg-docs pes_base       # tabular/pes_base
@update-pkg-docs pes_ql         # tabular/pes_ql
@update-pkg-docs pes_dql        # tabular/pes_dql
@update-pkg-docs pes_dqn        # ml/pes_dqn
@update-pkg-docs pes_a2c        # ml/pes_a2c
@update-pkg-docs pes_trf        # ml/pes_trf
@update-pkg-docs pes_rdqn       # ml/pes_rdqn
@update-pkg-docs pes_ens        # ml/pes_ens
@update-pkg-docs all
```
...
| Item | Path |
|------|------|
| Package root | `<GROUP>/<PKG>/` |
| Config | `<GROUP>/<PKG>/config/CONFIG.py` |
| Init / exports | `<GROUP>/<PKG>/__init__.py` |
...
1. List the contents of `<GROUP>/<PKG>/`, `<GROUP>/<PKG>/ext/`,
   `<GROUP>/<PKG>/src/`, and `<GROUP>/<PKG>/doc/` to discover all files.
...
1. List `<GROUP>/<PKG>/doc/*.md` to discover all Markdown documentation
   files.
...
- Write in **English**.
- Keep the README under ~200 lines; link out to `<group>/<pkg>/doc/*.md`
  for depth.
