...
h1/
  ...
  general/   # cross-model OOD benchmark harness + comparison doc
h2/
  tabular_uq/
...
> `__init__.py` at their own level). Run commands and static checks with
> the corresponding `h1\<GROUP>\<PKG>` path from the workspace root, or
> `cd h1` before any `python -m ...` invocation.

Throughout this prompt the short package name (`pes_dqn`) refers to its
full workspace path (`h1/ml/pes_dqn`, unless auditing `h2`). Imports use
the group prefix (`from ml.pes_dqn import ...`) and runs use
`python -m ml.pes_dqn` from within `h1/`.
...
```text
@audit-project pes_base       # h1/tabular/pes_base
@audit-project pes_ql         # h1/tabular/pes_ql
@audit-project pes_dql        # h1/tabular/pes_dql
@audit-project pes_dqn        # h1/ml/pes_dqn
...
this canonical order (skip `h2` unless the user explicitly includes it):
...
| Item | Path |
|------|------|
| Init / exports | `<LINE>/<GROUP>/<PKG>/__init__.py` |
| Config | `<LINE>/<GROUP>/<PKG>/config/CONFIG.py` |
| Entry point | `<LINE>/<GROUP>/<PKG>/__main__.py` |
| Core | `<LINE>/<GROUP>/<PKG>/ext/*.py` |
...
3. Static typing and lint: open and read
   [`.github/prompts/lint-and-typecheck.prompt.md`](./lint-and-typecheck.prompt.md)
   and follow its steps for the package instead of re-running pyright/pylint
   manually here — it is the single canonical source for that loop and its
   target thresholds.
...
| `linux_mpes_env` / `.sh` variant anywhere in `.py`, `.md`, or `utils/` | Stale — the project is Windows-only; remove unless the user explicitly reintroduces Linux support. |
| Missing `VIRTUAL_ENV` / `PYTHONIOENCODING` / `TF_ENABLE_ONEDNN_OPTS` setup in launcher scripts | Required for non-interactive runs. |
...
- After Python edits: re-run the loop described in
  [`lint-and-typecheck.prompt.md`](./lint-and-typecheck.prompt.md) on the
  affected package(s).
...
5. Quick import smoke-test for each `h1` package (run from within `h1/`):
   ...
   foreach ($p in $pkgs) {
     python -c "import importlib; m=importlib.import_module('$p'); print('OK', m.__name__)"
   }
   ```
...
- Both `snake_case` and `PascalCase` in scientific code — accepted by
  project convention.
- `h1/` and `h2/` having no `__init__.py` — by design, they are plain
  directories, not packages; do not add one.
