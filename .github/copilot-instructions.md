...
> `pes_rdqn` and `pes_trf` optimise only training hyperparameters (14); their
> architecture was fixed ad hoc in `config/CONFIG.py` because searching it was too costly.
> For the five optimised ensembles, `inputs/best_params.json` is a copy of the best `inputs/*_BAYESIAN_OPT/` run;
> `evaluate_ens._find_best_params` loads the file with the highest `value` among them.
...
- Every `doc/` directory contains `<pkg>_explained.md` (usage guide) and
  `<pkg>_theory.md` (theoretical foundations). Markdown is the only supported
  documentation format; never generate or maintain HTML exports.
