...
Una \emph{Kernel Density Estimator} (KDE) es un estimador no
param\'etrico de densidad que representa la distribuci\'on de una
muestra mediante la suma de funciones n\'ucleo centradas en cada
observaci\'on. Para una muestra unidimensional
...
controla el grado de suavizado. En TPE, estas estimaciones se
construyen por separado para las configuraciones con rendimiento
inferior a $y^{*}$ y para el resto; la raz\'on entre ambas densidades
orienta la selecci\'on de nuevos hiperpar\'ametros.

\subsection{Distribuci\'on de entrenamiento y generalizaci\'on}
...
la pol\'itica aprendida. Una evaluaci\'on
\emph{in-distribution} (ID) conserva las mismas leyes de generaci\'on
que el entrenamiento, de modo que

\begin{equation}
P_{\mathrm{test}} = P_{\mathrm{train}}
\label{eq:in-distribution}
\end{equation}

Una evaluaci\'on \emph{out-of-distribution} (OOD), en cambio, modifica
una o m\'as propiedades de esa distribuci\'on, por lo que
$P_{\mathrm{test}} \neq P_{\mathrm{train}}$. El cambio puede afectar
la distribuci\'on de estados o entradas (\emph{covariate shift}), la
relaci\'on entre entradas y recompensas, o la din\'amica de transici\'on.
En consecuencia, el rendimiento OOD mide la capacidad de
generalizaci\'on y robustez de la pol\'itica, y no solamente su ajuste
a las condiciones conocidas durante el entrenamiento.
...
\begin{description}
    ...
    \end{equation}
    \item[Significancia:] El test $t$ de Welch~\citep{Welch1947}
    contrasta si la diferencia entre las medias de $A$ y $B$ es
    compatible con la variabilidad observada, sin asumir varianzas
    iguales. Los grados de libertad se estiman mediante la correcci\'on
    de Welch--Satterthwaite. El valor $p$ de Welch es la probabilidad,
    bajo la hip\'otesis de medias iguales, de observar una diferencia
    tan extrema como la obtenida; un valor peque\~no de $p$ indica
    evidencia estad\'istica contra esa hip\'otesis, mientras que no
    demuestra por s\'i solo relevancia pr\'actica.
    \item[Cambio de pol\'itica:] La divergencia de Kullback--Leibler
    mide cu\'anto se aleja la distribuci\'on de acciones de $A$ respecto
    de la de $B$:
    $D_{\mathrm{KL}}(p_A \,\|\, p_B) =
    \sum_a p_A(a)\log\frac{p_A(a)}{p_B(a)}$.
    Vale $0$ cuando ambas pol\'iticas coinciden y aumenta cuanto mayor
    es el cambio en sus decisiones; no es sim\'etrica, por lo que
    $D_{\mathrm{KL}}(p_A \,\|\, p_B)$ no tiene por qu\'e coincidir con
    $D_{\mathrm{KL}}(p_B \,\|\, p_A)$.
\end{description}
