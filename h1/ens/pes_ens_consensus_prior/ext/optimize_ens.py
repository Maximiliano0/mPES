"""Bayesian optimization for the consensus ensemble with severity prior."""
import argparse
import importlib
import json
import os
...
def _objective(trial: optuna.Trial, trials_per_sequence: numpy.ndarray,
    ...
    params['agreement_bonus'] = trial.suggest_float('agreement_bonus', 0.0, 3.0)
    params['disagreement_penalty'] = trial.suggest_float('disagreement_penalty', 0.0, 2.0)
    params['prior_weight'] = trial.suggest_float('prior_weight', 0.0, 0.25)
    params['prior_sigma'] = trial.suggest_float('prior_sigma', 0.75, 3.0)
    ensemble = ConfidenceConsensusEnsemble(
        {name: params[f'weight_{name}'] for name in ('dqn', 'a2c', 'rdqn', 'trf')},
        params['confidence_power'], params['agreement_bonus'], params['disagreement_penalty'],
        params['prior_weight'], params['prior_sigma'])
...
def main() -> None:
    ...
    study_name = 'pes_ens_consensus_prior'
