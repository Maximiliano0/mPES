...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Evaluate the accQ ensemble.')
    ...
    subject_id = f'{datetime.now():%Y-%m-%d}_ENS_ACCQ'
    ...
    json_path, png_path = generate_results_report(
        subject_id, output_path, performances, performances_by_block, resource_data, 'PES_ENS_ACCQ_')
    numpy.save(os.path.join(output_path, f'PES_ENS_ACCQ_performances_{subject_id}.npy'), numpy.asarray(performances))
    _save_responses(output_path, f'PES_ENS_ACCQ_responses_{subject_id}.csv', recorded_actions)
