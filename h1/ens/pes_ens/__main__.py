"""
Main entry point for the pes_ens experiment.

Defines the main() function that orchestrates the full experiment lifecycle:
validation of every enabled ensemble-member model file, session creation
with logging, block/sequence/trial assignment, ensemble decision
collection via pygameMediator, severity updates, performance calculation,
and result report generation (JSON + PNG).
...
Configuration
-------------
All experiment parameters come from config/CONFIG.py, including the
``ENS_MEMBER_MODELS`` list that selects which sibling packages
...
## Internal Imports ##
######################
from . import (AGGREGATION_METHOD, ANSI, ALLOCATABLE_RESOURCES_PER_SEQUENCE,
               AVAILABLE_RESOURCES_PER_SEQUENCE,
               INITIAL_SEVERITY_FILE, INIT_NO_OF_CITIES, INPUTS_PATH,
               MAX_ALLOCATABLE_RESOURCES, MAX_INIT_RESOURCES, MAX_INIT_SEVERITY,
               MIN_ALLOCATABLE_RESOURCES, MIN_INIT_RESOURCES, MIN_INIT_SEVERITY,
               NUM_ATTEMPTS_TO_ASSIGN_SEQ, NUM_BLOCKS, NUM_MAX_TRIALS, NUM_MIN_TRIALS,
               NUM_SEQUENCES, OUTPUT_FILE_PREFIX, OUTPUTS_PATH, PLAYER_TYPE,
               RANDOM_INITIAL_SEVERITY, SAVE_INITIAL_SEVERITY_TO_FILE, SAVE_RESULTS,
               STARTING_BLOCK_INDEX, STARTING_SEQ_INDEX, TOTAL_NUM_TRIALS_IN_BLOCK,
               TRUST_MAX, USE_FIXED_BLOCK_SEQUENCES)
from .config.CONFIG import ENS_MEMBER_MODELS
...
call_nominated_aggregator = {
    'confidence_weighted_median': exp_utils.get_confidence_weighted_median,
...
def main():
    """Orchestrate the full pes_ens experiment lifecycle."""

    global Responses_filehandle

    # Validate every enabled ensemble-member model file (no training step).
    if PLAYER_TYPE == 'ENS_AGENT':
        terminal_utils.header("ENSEMBLE AGENT VALIDATION", width=80)
        terminal_utils.info("Checking ensemble member model files...")

        pkg_root = os.path.dirname(INPUTS_PATH)
        any_missing = False
        for member_cfg in ENS_MEMBER_MODELS:
            if not member_cfg.get('enabled', True):
                continue
            abs_path = os.path.normpath(os.path.join(pkg_root, member_cfg['path']))
            if not os.path.isfile(abs_path):
                terminal_utils.error(
                    f"Member '{member_cfg['name']}' model file not found!")
                terminal_utils.list_item(f"Expected path: {abs_path}", level=2)
                any_missing = True
                continue

            try:
                # safe_mode=False is required for the pes_trf model (Lambda
                # layer) and harmless for the others.  Trusted because
                # produced by our own training pipeline.
                model = tf.keras.models.load_model(abs_path, safe_mode=False)
                terminal_utils.success(
                    f"Member '{member_cfg['name']}' loaded ({member_cfg['role']})")
                terminal_utils.list_item(f"Parameters: {model.count_params()}", level=2)
                terminal_utils.list_item(f"Weight: {member_cfg['weight']}", level=2)
                del model  # pygameMediator will reload via EnsembleAgent
            except Exception as e:    # pylint: disable=broad-except
                terminal_utils.error(
                    f"Failed to load member '{member_cfg['name']}'!")
                terminal_utils.list_item(f"Error: {str(e)}", level=2)
                any_missing = True

        if any_missing:
            print("\nPlease train the affected sibling packages first.\n")
            return

        terminal_utils.success("All ensemble member files validated successfully!")
        print()
    ...
    if SAVE_RESULTS:
        ...
        SubjectInfo_filename = os.path.join(
            session_outputs_path,
            f'{OUTPUT_FILE_PREFIX}{MySubjectId}.txt')
        ...
        config_params = [
            ('ALLOCATABLE_RESOURCES_PER_SEQUENCE', ALLOCATABLE_RESOURCES_PER_SEQUENCE),
        ...
        NumTrialsPerSequence_list = []

        # NOTE: The following structure could be erased (?)
        ...
        if RANDOM_INITIAL_SEVERITY:
            ...
            if SAVE_INITIAL_SEVERITY_TO_FILE:
                numpy.savetxt(InitialSeverityCsv, first_severity, fmt='%d', delimiter=',')
        else:
        ...
        MyPerformances = []
        # A blockindex-by-playerindex array (only other players). Initial value: TRUST_MAX (i.e. full trust)
        TrustRatings = numpy.array([1, TRUST_MAX])
            ...
            for CurrentSequenceIndex, CurrentSequenceMapIndex in enumerate(CurrentBlockMapIndices):
                ...
                if CurrentBlockIndex == STARTING_BLOCK_INDEX and CurrentSequenceIndex < STARTING_SEQ_INDEX:
                    AbsoluteTrialCount += int(
                        NumTrials__blocks_x_sequences__2darray[CurrentBlockIndex, CurrentSequenceIndex])
                    AbsoluteSequenceIndex += 1
                    continue

                resources_to_allocate = ALLOCATABLE_RESOURCES_PER_SEQUENCE

                log_utils.tee(
                    f"  └─ Secuencia {AbsoluteSequenceIndex + 1}/{total_number_of_sequences} "
                    f"(Mapa #{int(CurrentSequenceMapIndex)}) - Progreso: "
                    f"{CurrentSequenceIndex + 1}/{NUM_SEQUENCES}"
                )
                ...
                # Initialise the map with the two pre-trial cities and their severities
                numpy.random.seed(3)
                ...
                resources_left = resources_to_allocate
                ...
                for trial_no in range(
                    ...
                    init_severity = SeveritiesOfCurrentlyVisibleCities
                    ...
                    log_utils.tee(
                        'Resources remaining: ', int(resources_left) if hasattr(
                            resources_left, 'numpy') else resources_left
                    )

                    new_locations = INIT_NO_OF_CITIES + trial_no + 1
                    ...
                    ResourceAllocationsAtCurrentlyVisibleCities.append(-1)

                    # Get response from Ensemble Agent
                    if resources_left > 0 and PLAYER_TYPE == 'ENS_AGENT':
                        (pc,
                         r,
                         rt_h,
                         rt_rel,
                         mov
                         ) = pygameMediator.provide_ens_agent_response(
                            ResourceAllocationsAtCurrentlyVisibleCities,
                            resources_left,
                            CurrentBlockIndex,
                    ...
                    if resources_left > 0:
                        ...
                        SeveritiesOfCurrentlyVisibleCities = exp_utils.get_updated_severity(
                            new_locations, ResourceAllocationsAtCurrentlyVisibleCities, init_severity)

                        if SeveritiesOfCurrentlyVisibleCities[-1] < init_severity[-1]:
                            direction.append(2)
                        else:
                            direction.append(1)
                ...
                _aggregated_allocations, aggregated_final_severity = call_nominated_aggregator(
                    AllMessages, first_severity, AbsoluteSequenceIndex, AbsoluteTrialCount)
    ...
    if PLAYER_TYPE == 'ENS_AGENT' and len(MyPerformances) > 0:
        try:
            ...
            resource_data = {
                'total_resources_per_sequence': ALLOCATABLE_RESOURCES_PER_SEQUENCE,
            ...
            try:
                ...
                log_utils.tee(
                    f"raw_mean_perf = {_raw_mean:.6f}  "
                    f"(std={_raw_std:.6f}, n={len(MyPerformances)})"
                )
                _bp_path = os.path.join(INPUTS_PATH, 'best_params.json')
                if os.path.isfile(_bp_path):
                    import json as _json
                    with open(_bp_path, 'r', encoding='utf-8') as _f:
                        _bp = _json.load(_f)
                    _expected = float(_bp.get('mean_perf', float('nan')))
                    if not numpy.isnan(_expected):
                        _delta = abs(_raw_mean - _expected)
                        log_utils.tee(
                            f"best_params.json mean_perf = {_expected:.6f}  "
                            f"|Δ| = {_delta:.6f}"
                        )
            except Exception as _cmp_err:  # pylint: disable=broad-except
                log_utils.tee(
                    f"Warning: could not compute raw_mean_perf comparison: {_cmp_err}"
                )
    ...
    if SAVE_RESULTS:
        exp_utils.exit_experiment_gracefully(
            ...
            MovementData=(
                os.path.join(
                    session_outputs_path,
                    f'{OUTPUT_FILE_PREFIX}movement_log_{MySubjectId}.npy'),
                total_movement),
            LogUtils=log_utils,
            _PygameMediator=pygameMediator
        )
