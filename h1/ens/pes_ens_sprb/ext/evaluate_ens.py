...
def main() -> None:
    """Run ensemble inference over the fixed sequences used by model evaluation."""
    ...
    json_path, png_path = generate_results_report(
        subject_id, output_path, performances, performances_by_block, resource_data, 'PES_ENS_SPRB_')
    numpy.save(os.path.join(output_path, f'PES_ENS_SPRB_performances_{subject_id}.npy'), numpy.asarray(performances))
    _save_responses(output_path, f'PES_ENS_SPRB_responses_{subject_id}.csv', recorded_actions)
