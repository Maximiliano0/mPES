"""Bayesian optimization for the Transformer-Guard ensemble."""
import argparse
import importlib
import json
import os
...
import optuna
import numpy
from .ensemble import TransformerGuardEnsemble
...
def _objective(trial: optuna.Trial, trials_per_sequence: numpy.ndarray,
    ...
    """Score one Transformer-Guard configuration in the environment."""
    ...
    params['trf_confidence_threshold'] = trial.suggest_float('trf_confidence_threshold', 0.1, 0.9)
    params['gate_slope'] = trial.suggest_float('gate_slope', 1.0, 20.0)
    ensemble = TransformerGuardEnsemble(
        {name: params[f'weight_{name}'] for name in ('dqn', 'a2c', 'rdqn', 'trf')},
        params['confidence_power'], params['trf_confidence_threshold'], params['gate_slope'])
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description='Optimize the Transformer-Guard ensemble.')
