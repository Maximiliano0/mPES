...
class ConfidenceConsensusEnsemble:
    """Select actions using confidence-weighted votes and model agreement."""

    def __init__(self, weights: dict[str, float] | None = None,
                 confidence_power: float = 1.0,
                 agreement_bonus: float = 0.5,
                 disagreement_penalty: float = 0.1) -> None:
    ...
    def predict(self, state: list[float] | numpy.ndarray, resources_left: int,
        ...
        """Return a consensus action, confidence and diagnostics."""
        ...
        for name in self._models:
            ...
            normalized_values[name] = _normalize(masked)
        ...
        for action_index in range(feasible + 1):
            ...
            scores[action_index] += sum(
                normalized_values[name][action_index] * confidences[name] for name in self._models)
        action = int(numpy.argmax(scores))
        confidence = float(numpy.clip(numpy.mean(list(confidences.values())), 0.0, 1.0))
        diagnostics['agreement'] = float(sum(confidence for name, confidence in confidences.items() if actions[name] == action))
