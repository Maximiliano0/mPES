...
> **Purpose** — Generalise and benchmark the six active trained mPES agents
> (every active package except `tabular/pes_base`) under a 22-scenario matrix
...
> **Pending regeneration.** The previous heatmaps, matrices, per-scenario
> histograms, and `benchmark_report.md` were produced together with `pes_ens`
> and have been removed along with its raw cell data. Re-run the workflow
> above (`orchestrate` → `aggregate` → `plot_matrix` → `report`) to regenerate
> them for the current 6-model catalogue.
...
| OOD degradation (Δ vs baseline) | `results/heatmap_ood_degradation.png` | `RdBu_r` (diverging) | symmetric around 0 |
