...
_PALETTE_ANCHORS = {
    ...
    'mpes_perf': ['#f3f7f4', '#cfe7e2', '#7fc6c0', '#3b9aa1', '#1f6e83',
                  '#1b455f', '#16263f'],
    # Diverging cool/warm for stress degradation (centered at zero).
    # Cold side = "less stressed than baseline", warm = "more stressed".
...
def plot_all() -> None:
    ...
    _heatmap(M_degr, models, scenarios,
             'Stress degradation: baseline_mean − cell_mean',
             os.path.join(RESULTS_DIR, 'heatmap_stress_degradation'),
             cmap='mpes_div', vmin=-bound, vmax=bound, fmt='{:+.3f}',
             cbar_label='\u0394 perf. (+ = more stressed)')
    ...
    if os.path.isfile(kl_csv):
        ...
        _heatmap(kbody_disp, kmodels, kscen,
                 'KL(action distribution \u2225 baseline policy)',
