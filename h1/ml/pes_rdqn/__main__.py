"""
Main entry point for the pes_rdqn experiment.

Defines the main() function that orchestrates the full experiment lifecycle:
validation of RDQN-Agent training files, session creation with logging,
block/sequence/trial assignment, RDQN-Agent decision collection via
pygameMediator, severity updates, performance calculation, and
result report generation (JSON + PNG).

This module is structurally identical to pes_ql/__main__.py.  The RDQN-specific
behaviour (neural-network training) is performed offline via ext/train_rdqn.py;
the resulting model is consumed here at experiment time.
...
def main():
    """Orchestrate the full pes_rdqn experiment lifecycle."""

    global Responses_filehandle

    # Validate RDQN-Agent training files if needed
    if PLAYER_TYPE == 'RDQN_AGENT':
        from .config.CONFIG import RDQN_MODEL_FILE
        model_file = os.path.join(INPUTS_PATH, RDQN_MODEL_FILE)
        rewards_file = os.path.join(INPUTS_PATH, 'rewards.npy')

        terminal_utils.header("RDQN-AGENT VALIDATION", width=80)
        terminal_utils.info("Checking training files...")

        if not os.path.isfile(model_file):
            terminal_utils.error("RDQN model file not found!")
            terminal_utils.list_item(f"Expected path: {model_file}", level=2)
            print("\nTo train the RDQN-Agent, run:")
            terminal_utils.list_item("python3 -m ml.pes_rdqn.ext.train_rdqn")
            print()
            return
        ...
        try:
            rdqn_model = tf.keras.models.load_model(model_file)
            rewards = numpy.load(rewards_file)
            terminal_utils.success("RDQN model loaded successfully")
            terminal_utils.list_item(f"Parameters: {rdqn_model.count_params():,}", level=2)
        ...
        for CurrentBlockIndex in range(NUM_BLOCKS):
            ...
            for CurrentSequenceIndex, CurrentSequenceMapIndex in enumerate(CurrentBlockMapIndices):
                ...
                for trial_no in range(
                    ...
                    # Get response from RDQN Agent
                    if resources_left > 0 and PLAYER_TYPE == 'RDQN_AGENT':
                        (pc,
                         r,
                         rt_h,
                         rt_rel,
                         mov
                         ) = pygameMediator.provide_rdqn_agent_response(
                            ResourceAllocationsAtCurrentlyVisibleCities,
                            resources_left,
                            CurrentBlockIndex,
    ...
    if PLAYER_TYPE == 'RDQN_AGENT' and len(MyPerformances) > 0:
        try:
            ...
            # ~3% from a Colab-GPU Optuna run because of cuDNN-LSTM kernel
            # differences — see pes_rdqn/ext/eval_model.py for a stand-alone
            # reproducer.
            try:
