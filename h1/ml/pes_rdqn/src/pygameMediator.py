"""RDQN Agent Game Display Mediator and Response Handler.

This module bridges the Pygame game interface with the trained RDQN agent
(Recurrent Deep Q-Network model), handling agent decision-making, response timing,
and confidence calculations. It manages the communication between the game
display and the RDQN agent by processing game states and generating
appropriately timed and confident responses.

Key Functions:
    - rdqn_agent_meta_cognitive: Meta-cognitive decision making with entropy-based confidence
    - provide_rdqn_agent_response: Main interface returning agent response and timing

Module Dependencies:
    External: numpy, tensorflow, os
    Internal: log_utils, convert_globalseq_to_seqs, CONFIG constants, rdqn_model
...
from ..config.CONFIG import RDQN_HISTORY_LEN
from ..ext.tools import convert_globalseq_to_seqs
from ..ext.rdqn_model import normalize_state, HistoryDeque
...
# Per-(session, sequence) sliding-window cache so the LSTM trunk receives
...
def provide_rdqn_agent_response(
    _resources,
    resources_left,
    session_no,
    sequence_no,
    ...
    """Generate RDQN agent response using trained Recurrent Deep Q-Network policy.

    Main interface for obtaining agent responses. Loads the trained RDQN model
    ...
    FileNotFoundError
        If RDQN model (.keras) or rewards file (rewards.npy) not found in INPUTS_PATH.
    RuntimeError
        If RDQN model or rewards files are corrupted and cannot be loaded.

    Notes
    -----
    - Requires RDQN model pre-training via: python3 -m ml.pes_rdqn.ext.train_rdqn
    ...
    # Load and validate RDQN model
    model_file = os.path.join(INPUTS_PATH, RDQN_MODEL_FILE)
    rewards_file = os.path.join(INPUTS_PATH, 'rewards.npy')

    if not os.path.isfile(model_file):
        raise FileNotFoundError(
            f"\nFATAL ERROR: RDQN model file not found at {model_file}\n"
            f"Please train the RDQN-Agent first by running: python3 -m ml.pes_rdqn.ext.train_rdqn\n"
        )
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Error: {str(e)}\n"
            f"Files may be corrupted. Please retrain by running: python3 -m ml.pes_rdqn.ext.train_rdqn\n"
        ) from e

    if VERBOSE:
        print("Reading preloaded RDQN model for RDQN-Agent")
    ...
    # Build state for RDQN forward pass
    try:
    ...
    # Normalise state and perform forward pass through RDQN.
    ...
    # Maintain a sliding window of past normalised states for the LSTM.
    cache_key = (int(session_no), int(sequence_no))
    history = _history_cache.get(cache_key)
    if history is None:
        history = HistoryDeque(RDQN_HISTORY_LEN, 3)
    ...
    q_values = rdqn_model(state_tensor, training=False).numpy().flatten()  # shape (11,)

    if VERBOSE:
        print(f"Q-values from RDQN: {q_values}")

    # Calculate the response and confidence using meta-cognitive entropy-based evaluation
    from ..ext.pandemic import rdqn_agent_meta_cognitive  # lazy import to avoid circular dependency
    ...
    if VERBOSE:
        print(f"RDQN-Agent Response: {resp}, Confidence: {confidence}")
