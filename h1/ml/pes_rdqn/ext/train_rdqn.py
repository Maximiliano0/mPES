'''
pes_rdqn - Pandemic Experiment Scenario: RDQN-Agent Training Pipeline

Trains a Recurrent Deep Q-Network agent on the Pandemic environment using
...
3. Train RDQN agent (default 30 000 episodes, configurable via CLI)
...
- RDQN-specific hyperparameters: hidden_units, batch_size, buffer_size,
  target_sync_freq
- Default episodes: 30 000 (configurable via CLI; see ``RDQN_EPISODES``)
...
::
    ...
    python -m ml.pes_rdqn.ext.train_rdqn [num_episodes]
...
##  Imports internos    ##
##########################
from .pandemic import Pandemic, rdqn_agent_meta_cognitive, run_experiment, RDQNTraining
...
from ..config.CONFIG import (SEED, RDQN_MODEL_FILE)
...
# Device selection is centralised in pes_rdqn/__init__.py via MPES_USE_GPU.
# Re-asserting it here only as a defensive fallback when this module is
# launched in isolation (e.g. ``python ext/train_rdqn.py``) without the
...
def _parse_cli_args():
    """
    Parse ``train_rdqn`` CLI arguments.
...
def _load_canonical_params():
    """Load best-trial params from the canonical ``pes_rdqn/inputs/best_params.json``.

    This file is written automatically by ``optimize_rdqn.main`` as a mirror of
    ...
    # if the run was cut short, which is fine — train_rdqn retrains from scratch).
    model_path = os.path.join(
        INPUTS_PATH, f'{opt_date}_BAYESIAN_OPT', f'rdqn_best_{opt_date}.keras'
    )
...
def _load_best_trial(opt_date):
    ...
    Resolution order (mirrors pes_ql/pes_dql for cross-package consistency):
      ...
      1. ``pes_rdqn/inputs/<opt_date>_BAYESIAN_OPT/best_params_<opt_date>.json``
         (small JSON sidecar written by ``optimize_rdqn.main``).
      2. ``pes_rdqn/inputs/<opt_date>_BAYESIAN_OPT/optuna_study_<opt_date>.db``
    ...
    model_path = os.path.join(opt_dir, f'rdqn_best_{opt_date}.keras')
    if not os.path.isfile(model_path):
        model_path = None
    ...
    if not os.path.isfile(db_file):
        raise FileNotFoundError(
            ...
            f"pes_rdqn/inputs/ first."
        )
    study = optuna.load_study(
        study_name=f'rdqn_opt_{opt_date}',
    ...
    if trial_seed is None:
        ...
        # the per-trial seed formula in optimize_rdqn.objective.
...
def main():
    """Run the full RDQN-Agent training and evaluation pipeline.

    Trains the RDQN agent from scratch using the hyperparameters defined in
    ``config/CONFIG.py``.  Usage: ``python -m ml.pes_rdqn.ext.train_rdqn [episodes]``.
    """

    # ---- Parse CLI arguments ------------------------------------------------
    cli_num_episodes, from_best_date = _parse_cli_args()

    header("RDQN-AGENT TRAINING PIPELINE", width=80)
    ...
    train_dir = os.path.join(INPUTS_PATH, f'{train_date}_RDQN_TRAIN')
    ...
    section("RDQN Training", width=80)

    # Window/LSTM always from CONFIG.py; the rest is overridden by inputs/best_params.json when present
    from ..config.CONFIG import (RDQN_LEARNING_RATE, RDQN_DISCOUNT,
                                 RDQN_EPSILON_INITIAL, RDQN_EPSILON_MIN,
                                 RDQN_HIDDEN_UNITS, RDQN_BATCH_SIZE,
                                 RDQN_REPLAY_BUFFER_SIZE, RDQN_TARGET_SYNC_FREQ,
                                 RDQN_MAX_GRAD_NORM, RDQN_PENALTY_COEFF,
                                 RDQN_WARMUP_RATIO, RDQN_TARGET_RATIO,
                                 RDQN_LEARNING_STARTS_FRAC, RDQN_EPISODES,
                                 RDQN_HISTORY_LEN, RDQN_LSTM_UNITS)

    best_info = None
    if from_best_date is None:
        # Resolution order (highest priority first):
        #   1. pes_rdqn/inputs/best_params.json  (canonical mirror, always up-to-date)
    ...
    if best_info is not None:
        ...
        hidden_units     = list(best_info['hidden_units']) if best_info['hidden_units'] \
                                                          else list(RDQN_HIDDEN_UNITS)
        batch_size       = bp['batch_size']
        buffer_size      = bp['buffer_size']
        target_sync_freq = bp['target_sync_freq']
        max_grad_norm    = bp.get('max_grad_norm', RDQN_MAX_GRAD_NORM)
        ...
        if bp.get('use_pbrs', bp.get('penalty_coeff', 0.0) > 0):
            penalty_coeff = float(bp.get('penalty_coeff', RDQN_PENALTY_COEFF))
        else:
            penalty_coeff = 0.0
        warmup_ratio        = bp.get('warmup_ratio',  RDQN_WARMUP_RATIO)
        target_ratio        = bp.get('target_ratio',  RDQN_TARGET_RATIO)
        learning_starts_frac = bp.get('learning_starts_frac', RDQN_LEARNING_STARTS_FRAC)
        learning_starts     = max(int(learning_starts_frac * buffer_size), int(batch_size))
        # Episode-count resolution:
        #   * CLI-supplied value wins (e.g. ``train_rdqn 40000`` for parity vs
        #     Optuna mean_perf).
        #   * Otherwise use CONFIG.RDQN_EPISODES (currently equal to the
        ...
        if num_episodes != opt_episodes:
            info(f"  Training with {num_episodes:,} episodes (CONFIG.RDQN_EPISODES) "
        ...
        learning_starts_frac = RDQN_LEARNING_STARTS_FRAC
        # Match the formula used in optimize_rdqn.objective() exactly.
        learning_starts  = max(int(learning_starts_frac * buffer_size), int(batch_size))
        num_episodes     = cli_num_episodes if cli_num_episodes is not None else RDQN_EPISODES

    info(f"Starting RDQN training ({num_episodes:,} episodes)...")
    info(f"Network: Input({RDQN_HISTORY_LEN}, 3) -> Masking -> LSTM({RDQN_LSTM_UNITS}) -> "
         f"{' -> '.join(f'Dense({u}, ReLU)' for u in hidden_units)} -> Dense(11, linear)")
    info(f"Replay buffer: {buffer_size:,}  |  Batch size: {batch_size}  |  Target sync: {target_sync_freq:,}")
    info(f"Seed:          {train_seed}")
    info("(This may take several minutes)")
    print()

    rewards, rdqn_model, confsrl = RDQNTraining(
        ...
        history_len=RDQN_HISTORY_LEN, lstm_units=RDQN_LSTM_UNITS)
    print()
    success("Training completed")
    list_item(f"Network parameters: {rdqn_model.count_params():,}")
    list_item(f"Rewards history length: {len(rewards)}")

    info("Saving trained model...")

    # Save RDQN model and rewards with date stamp
    model_file = os.path.join(train_dir, f'rdqn_model_{train_date}.keras')
    ...
    with open(config_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 80 + "\n")
        f.write(f"Network Parameters:          {rdqn_model.count_params():,}\n")
        ...
        f.write("-" * 80 + "\n")
        f.write(f"Model File:                  rdqn_model_{train_date}.keras\n")
        f.write(f"Rewards File:                rewards_{train_date}.npy\n")
        f.write(f"Configuration File:          training_config_{train_date}.txt\n\n")

        f.write("DESCRIPTION\n")
        f.write("-" * 80 + "\n")
        f.write("Files saved from RDQN training on the Pandemic Scenario.\n")
    ...
    success(f"✓ Model saved to rdqn_model_{train_date}.keras")
    ...
    if rewards:
        ...
        ax.set_title('RDQN Training: Average Reward Progression', fontsize=14, fontweight='bold', pad=20)
        ax.grid(True, alpha=0.3)
        ax.legend(loc='best', fontsize=11)
        plt.tight_layout()
        plt.savefig(os.path.join(train_dir, f'rdqn_agent_rewards_vs_episodes_{train_date}.png'), dpi=150,
                    bbox_inches='tight')
        plt.close()
        list_item(f"Saved: rdqn_agent_rewards_vs_episodes_{train_date}.png")
    else:
        info("No rewards history available — skipping reward plot")
    print()

    if True:  # pylint: disable=using-constant-test
        section("RDQN-Agent Evaluation", width=80)
        ...
        # ── Parity evaluation: matches optimize_rdqn.objective.qf exactly ──
        ...
        _parity_history = HistoryDeque(RDQN_HISTORY_LEN, 3)
        _parity_state = {'seqid': -1}

        def parity_qf(_env, state, seqid):
            if seqid != _parity_state['seqid']:
                _parity_history.reset()
                _parity_state['seqid'] = seqid
            norm_s = normalize_state(state, _max_res, _max_seq, _max_sev)
            _parity_history.append_step(norm_s)
            window = _parity_history.current_window()
            q_vals = rdqn_model(window[numpy.newaxis], training=False).numpy()[0].copy()
        ...
        if best_info is not None:
            ...
            # Tolerance bands account for cuDNN-LSTM (Colab GPU) vs generic
            # CPU LSTM drift, which empirically reaches ~3% mean-perf delta
            # even with identical weights / seeds.  Anything above 5% likely
            # indicates a real bug (wrong weights, wrong sequences, etc.).
            if _delta < 1e-6:
                _verdict = 'OK (bit-exact)'
            elif _delta < 1e-3:
                _verdict = 'within float tolerance'
            elif _delta < 5e-2:
                _verdict = 'GPU↔CPU LSTM drift (expected)'
            else:
        ...
        _eval_history = HistoryDeque(RDQN_HISTORY_LEN, 3)
        _eval_state = {'seqid': -1}

        def eval_qf(_env, state, seqid):
            """Select the best action using the trained Q-network with meta-cognitive confidence."""
            if seqid != _eval_state['seqid']:
                _eval_history.reset()
                _eval_state['seqid'] = seqid
            norm_s = normalize_state(state, _env.max_resources,
                                     _env.max_seq_length, _env.max_severity)
            _eval_history.append_step(norm_s)
            window = _eval_history.current_window()
            q_vals = rdqn_model(window[numpy.newaxis], training=False).numpy()[0].copy()

            _response, confidence, _rt_hold, _rt_release = rdqn_agent_meta_cognitive(
                q_vals, state[0], 10000
            )
        ...
        ax.plot(seqs, color='#d62728', linewidth=2.5, marker='o', markersize=5, label='RDQN-Agent')
        ax.set_xlabel('Trial', fontsize=12, fontweight='bold')
        ax.set_ylabel('Final Severity Achieved', fontsize=12, fontweight='bold')
        ax.set_title('RDQN-Agent Evaluation: Severity per Sequence', fontsize=14, fontweight='bold', pad=20)
        ax.grid(True, alpha=0.3)
        ax.legend(loc='best', fontsize=11)
        plt.tight_layout()
        plt.savefig(
            os.path.join(
                train_dir,
                f'rdqn_agent_sequence_performance_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
        list_item(f"Saved: rdqn_agent_sequence_performance_{train_date}.png")

        _fig, ax = plt.subplots(figsize=(12, 6))
        ax.plot(perfs, color='#9467bd', linewidth=2.5, marker='s', markersize=5, label='RDQN-Agent')
        ax.set_ylabel('Normalised Performance (0-1)', fontsize=12, fontweight='bold')
        ax.set_xlabel('Trial', fontsize=12, fontweight='bold')
        ax.set_title('RDQN-Agent Evaluation: Normalised Performance per Sequence',
        ...
        plt.savefig(
            os.path.join(
                train_dir,
                f'rdqn_agent_normalised_performance_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
        list_item(f"Saved: rdqn_agent_normalised_performance_{train_date}.png")

        cumperfs = numpy.cumsum(perfs)
        Domain = numpy.arange(1, 1 + 64)
        _fig, ax = plt.subplots(figsize=(12, 6))
        ax.plot(cumperfs / Domain, color='#8c564b', linewidth=2.5, marker='^', markersize=5, label='RDQN-Agent')
        ax.set_ylabel('Cumulative Normalised Performance', fontsize=12, fontweight='bold')
        ax.set_xlabel('Trial', fontsize=12, fontweight='bold')
        ax.set_title('RDQN-Agent Evaluation: Cumulative Performance Trend',
                      fontsize=14, fontweight='bold', pad=20)
        ...
        plt.savefig(
            os.path.join(
                train_dir,
                f'rdqn_agent_cumulative_performance_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
        list_item(f"Saved: rdqn_agent_cumulative_performance_{train_date}.png")
        ...
        ax.scatter(
            ...
            edgecolors='navy',
            linewidth=0.5)
        ax.set_title('RDQN-Agent: Decision Confidence Scores During Evaluation',
                      fontsize=14, fontweight='bold', pad=20)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(train_dir, f'rdqn_agent_confidences_{train_date}.png'), dpi=150, bbox_inches='tight')
        plt.close()
        list_item(f"Saved: rdqn_agent_confidences_{train_date}.png")
        ...
        ax.set_xlabel('Trial Number', fontsize=12, fontweight='bold')
        ax.set_title('RDQN-Agent: Normalised Confidence Scores', fontsize=14, fontweight='bold', pad=20)
        ax.set_ylim(-0.1, 1.1)
        ax.set_xlim(-10, 360)
        ax.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        plt.savefig(
            os.path.join(
                train_dir,
                f'rdqn_agent_remapped_confidences_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
    ...
    success("RDQN-Agent training pipeline finished successfully!")
    info(f"Output directory: {train_dir}")
    print()
#
# END OF 'main()
