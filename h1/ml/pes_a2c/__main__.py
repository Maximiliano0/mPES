"""
Main entry point for the pes_a2c experiment.

Defines the main() function that orchestrates the full experiment lifecycle:
validation of the A2C Actor model file, session creation with logging,
block/sequence/trial assignment, A2C Agent decision collection via
pygameMediator, severity updates, performance calculation, and
result report generation (JSON + PNG).

The A2C-specific behaviour (training the neural network) is performed offline
via ext/train_a2c.py; the resulting Keras Actor model is consumed here at
experiment time.
...
def main():
    """Orchestrate the full pes_a2c experiment lifecycle."""

    global Responses_filehandle

    # Validate A2C Actor model and rewards file if needed
    if PLAYER_TYPE in ('RL_AGENT', 'DQN_AGENT', 'A2C_AGENT'):
        model_file = os.path.join(INPUTS_PATH, AC_MODEL_ACTOR_FILE)
        rewards_file = os.path.join(INPUTS_PATH, 'rewards.npy')

        terminal_utils.header("A2C AGENT VALIDATION", width=80)
        terminal_utils.info("Checking A2C model files...")

        if not os.path.exists(model_file):
            terminal_utils.error("A2C Actor model file not found!")
            terminal_utils.list_item(f"Expected path: {model_file}", level=2)
            print("\nTo train the A2C Agent, run:")
            terminal_utils.list_item("python3 -m ml.pes_a2c.ext.train_a2c")
            print()
            return
        ...
        try:
            ...
            terminal_utils.success("A2C Actor model loaded successfully")
            terminal_utils.list_item(f"Parameters: {model.count_params()}", level=2)
            terminal_utils.list_item(f"Output shape: {model.output_shape}", level=2)
            ...
            print()
            del model  # free memory; pygameMediator will reload when needed
        except Exception as e:
            terminal_utils.error("Failed to load A2C model!")
        ...
        for CurrentBlockIndex in range(NUM_BLOCKS):
            ...
            for CurrentSequenceIndex, CurrentSequenceMapIndex in enumerate(CurrentBlockMapIndices):
                ...
                for trial_no in range(
                    ...
                    # Get response from A2C Agent
                    if resources_left > 0 and PLAYER_TYPE in ('RL_AGENT', 'DQN_AGENT', 'A2C_AGENT'):
                        (pc,
                         r,
                         rt_h,
                         rt_rel,
                         mov
                         ) = pygameMediator.provide_ac_agent_response(
                            ResourceAllocationsAtCurrentlyVisibleCities,
                            resources_left,
                            CurrentBlockIndex,
    ...
    if PLAYER_TYPE in ('RL_AGENT', 'DQN_AGENT', 'A2C_AGENT') and len(MyPerformances) > 0:
        try:
