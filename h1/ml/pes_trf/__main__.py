"""
Main entry point for the pes_trf experiment.

Defines the main() function that orchestrates the full experiment lifecycle:
validation of TRF-Agent training files, session creation with logging,
block/sequence/trial assignment, TRF-Agent decision collection via
pygameMediator, severity updates, performance calculation, and
result report generation (JSON + PNG).

This module is structurally identical to pes_ql/__main__.py.  The TRF-specific
behaviour (neural-network training) is performed offline via ext/train_transformer.py;
...
def main():
    """Orchestrate the full pes_trf experiment lifecycle."""

    global Responses_filehandle

    # Validate TRF-Agent training files if needed
    if PLAYER_TYPE == 'TRF_AGENT':
        ...
        terminal_utils.header("TRF-AGENT VALIDATION", width=80)
        terminal_utils.info("Checking training files...")

        if not os.path.isfile(model_file):
            terminal_utils.error("TRF model file not found!")
            terminal_utils.list_item(f"Expected path: {model_file}", level=2)
            print("\nTo train the TRF-Agent, run:")
            terminal_utils.list_item("python3 -m ml.pes_trf.ext.train_transformer")
            print()
            return
        ...
        try:
            ...
            terminal_utils.success("TRF model loaded successfully")
            terminal_utils.list_item(f"Parameters: {trf_model.count_params():,}", level=2)
        ...
        for CurrentBlockIndex in range(NUM_BLOCKS):
            ...
            for CurrentSequenceIndex, CurrentSequenceMapIndex in enumerate(CurrentBlockMapIndices):
                ...
                for trial_no in range(
                    ...
                    # Get response from TRF Agent
                    if resources_left > 0 and PLAYER_TYPE == 'TRF_AGENT':
                        (pc,
                         r,
                         rt_h,
                         rt_rel,
                         mov
                         ) = pygameMediator.provide_trf_agent_response(
                            ResourceAllocationsAtCurrentlyVisibleCities,
                            resources_left,
                            CurrentBlockIndex,
    ...
    if PLAYER_TYPE == 'TRF_AGENT' and len(MyPerformances) > 0:
        try:
