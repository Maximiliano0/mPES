"""
pes_trf - Pandemic Experiment Scenario
...
suppressed before argmax) so that the metric matches the behaviour of the TRF agent
...
Usage:
    python3 -m ml.pes_trf.ext.optimize_tr [n_trials] [--resume YYYY-MM-DD]
...
##  Imports internos    ##
##########################
from .pandemic import Pandemic, run_experiment, TRFTraining, trf_agent_meta_cognitive
from .transformer_model import build_q_network, normalize_state, HistoryDeque
from ..src.terminal_utils import header, section, success, info, list_item
from .tools import convert_globalseq_to_seqs
from ..config.CONFIG import (SEED, TRF_LEARNING_RATE, TRF_DISCOUNT,
                             TRF_EPSILON_INITIAL, TRF_EPSILON_MIN,
                             TRF_EPISODES, TRF_HIDDEN_UNITS, TRF_BATCH_SIZE,
...
# is launched in isolation (e.g. ``python ext/optimize_tr.py``) without the
...
def objective(trial: optuna.Trial) -> float:
    """Optuna objective for Transformer-DQN hyperparameter search.

    Trains a TRF agent with sampled hyperparameters and returns the mean
    normalised performance over the fixed 64-sequence evaluation set.

    Parameters
    ----------
    trial : optuna.Trial
        Active Optuna trial (used to suggest hyperparameters and to
        report intermediate rewards for pruning).

    Returns
    -------
    float
        Mean normalised performance over the evaluation sequences
        (higher is better; the study direction is ``maximize``).
    """
    # --- Sample hyperparameters ---
    # Widened search space (post Double-TRF + masking + learning_starts fix).
    # Rationale documented in pes_trf/doc/ — boundaries opened where the
    ...
    # (TRF warm-up). Converted to absolute count below.
    ...
    # ``train_transformer.py --from-best <date>`` (reads trial_seed user_attr below).
    trial_seed = SEED + int(trial.number) + 1

    rewards, model, _ = TRFTraining(
    ...
    # Persist the per-trial seed so train_transformer.py --from-best can reproduce it
    ...
    # every trial — and any subsequent fresh call to ``train_transformer.py`` —
...
def _save_report(study, opt_dir, opt_date, best_model, best_rewards):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        # should always retrain at the full ``TRF_EPISODES`` to match the saved
        # ``trf_best_<date>.keras`` (which is also retrained at full episodes).
    ...
    ax.set_title('Bayesian Optimisation: Convergence (TRF)', fontsize=14, fontweight='bold', pad=20)
    ...
    try:
        ...
        ax.set_title('TRF Hyperparameter Importance', fontsize=14, fontweight='bold', pad=20)
    ...
    best_model.save(os.path.join(opt_dir, f'trf_best_{opt_date}.keras'))
    numpy.save(os.path.join(opt_dir, f'rewards_best_{opt_date}.npy'), best_rewards)
    success(f"Best model saved: trf_best_{opt_date}.keras")
    ...
    # --- Mirror to the standard input path so train_transformer.py picks it up
...
def main():
    """Run Bayesian optimisation of TRF hyperparameters using Optuna."""

    header("BAYESIAN OPTIMISATION — TRF HYPERPARAMETERS", width=80)
    ...
    info("Final winner is retrained at full TRF_EPISODES after study.optimize().")
    print()
    ...
    if len(study.trials) == 0:
        ...
        # retrained at full TRF_EPISODES afterwards.
        ...
        study.enqueue_trial({
            ...
            'use_pbrs': bool(TRF_PENALTY_COEFF > 0),
    ...
    section("Best TRF Model", width=80)
    ...
    if _best_artifacts['weights'] is not None and _best_artifacts['value'] >= best.value:
        ...
        best_rewards_list, best_model, _ = TRFTraining(
            env_final,
            bp['learning_rate'],
            bp['discount_factor'],
            bp['epsilon_initial'],
            ...
            penalty_coeff=bp.get('penalty_coeff', TRF_PENALTY_COEFF if bp.get('use_pbrs', True) else 0.0),
            compute_confidence=False,
