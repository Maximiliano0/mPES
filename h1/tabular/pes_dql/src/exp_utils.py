...
# reserves 9 resources as "preassigned" (see Pandemic.__init__: max_resources
...
def _evolve_single_city(initial_severity, allocation, num_evolutions):
    ...
    once. ``num_evolutions`` is the number of such updates the city undergoes
    by the end of the sequence (i.e. ``L - c`` for a length-``L`` sequence).
    ...
    allocation : int
        Constant resource allocation for this city (0..MAX_ALLOCATABLE_RESOURCES).
    num_evolutions : int
...
def _best_feasible_sequence_severity(InitialSequenceSeverities,
    ...
    Solves the per-sequence allocation problem exactly: for each city ``c``,
    pick an integer allocation ``a_c`` in ``[0, max_alloc]`` such that
    ``sum(a_c) <= budget`` and the sum of per-city final severities is
    minimised. Because each city's final severity depends only on its own
    allocation (the dynamics are decoupled across cities), this reduces to a
    bounded-knapsack DP with ``L * (budget+1) * (max_alloc+1)`` work.

    Replaces the previous reference value, which assumed allocating
    ``max_alloc`` to every trial — infeasible for sequences longer than
    ``budget // max_alloc`` and therefore producing a "best case" the agent
    could never reach. Using the true feasible optimum makes the normalised
    performance metric reach 1.0 only for an actually-attainable policy.
    ...
    # per_city[c][a] = final severity of city c if allocated a units
    per_city = [
...
def calculate_normalised_final_severity_performance_metric( SeveritiesFromSequence, InitialSequenceSeverities ):
    ...
    tuple
        ...
        - BestCaseSequenceSeverity (float): Sum of per-city final severities under
          the optimal *feasible* allocation (DP over the per-sequence budget,
          see ``_best_feasible_sequence_severity``). A score of 1.0 corresponds
          to playing this oracle policy.
    """
    ...
    # per-sequence budget, NOT "max_alloc to every trial" — that reference
    # required up to L * MAX_ALLOC resources (e.g. 100 for L=10) while the
    # agent only has _FEASIBLE_BUDGET_PER_SEQUENCE (=30) to spend, capping
    # the achievable normalised score below 1.0 for L > budget/MAX_ALLOC.
