...
######################
from . import (
    AGGREGATION_METHOD, ANSI, AVAILABLE_RESOURCES_PER_SEQUENCE, INIT_NO_OF_CITIES, INITIAL_SEVERITY_FILE,
    INPUTS_PATH, MAX_ALLOCATABLE_RESOURCES, MAX_INIT_RESOURCES, MAX_INIT_SEVERITY, MAX_SEVERITY,
    MIN_ALLOCATABLE_RESOURCES, MIN_INIT_RESOURCES, MIN_INIT_SEVERITY, NUM_ATTEMPTS_TO_ASSIGN_SEQ,
    NUM_BLOCKS, NUM_MAX_TRIALS, NUM_MIN_TRIALS, NUM_SEQUENCES, OUTPUT_FILE_PREFIX, OUTPUTS_PATH,
    PLAYER_TYPE, RANDOM_INITIAL_SEVERITY, SAVE_INITIAL_SEVERITY_TO_FILE, SAVE_RESULTS,
    STARTING_BLOCK_INDEX, STARTING_SEQ_INDEX, TOTAL_NUM_TRIALS_IN_BLOCK, TRUST_MAX, USE_FIXED_BLOCK_SEQUENCES
)
...
def main():
    ...
    # Default-initialise variables that may stay unbound when SAVE_RESULTS is False
    MySubjectId: str = ""
    session_outputs_path: str = ""
    MyMessage = numpy.empty(0)
    MyPerformances: list = []
    total_movement: dict = {}
    ...
    if PLAYER_TYPE == 'RL_AGENT':
        ...
        if not os.path.isfile( q_file ):
            ...
            terminal_utils.list_item("python3 -m tabular.pes_dql.ext.train_rl")
            print()
            return
        ...
        try:
            ...
            # Validate Q-table dimensionality matches the current CONFIG.
            # The first dim depends on the per-sequence budget after the
            # initial mandatory allocations (AVAILABLE_RESOURCES_PER_SEQUENCE
            # − initial-allocations); the remaining three dims are determined
            # entirely by CONFIG.  A stale q.npy from a sibling package
            # (different budget / action cap) would silently produce garbage
            # without this check.
            # NOTE: severity_states = MAX_SEVERITY + 1, NOT MAX_INIT_SEVERITY + 1.
            # The Q-table spans all post-multiplication severities [0..MAX_SEVERITY].
            expected_tail = (NUM_MAX_TRIALS + 1, MAX_SEVERITY + 1, MAX_ALLOCATABLE_RESOURCES + 1)
            if Q.ndim != 4 or Q.shape[1:] != expected_tail:
                terminal_utils.error("Q-Table shape is incompatible with current CONFIG!")
                terminal_utils.list_item(f"Got:      {Q.shape}", level=2)
    ...
    if SAVE_RESULTS:
        ...
        for CurrentBlockIndex in range( NUM_BLOCKS ):
            ...
            for CurrentSequenceIndex, CurrentSequenceMapIndex in enumerate( CurrentBlockMapIndices ):
                ...
                # Use an isolated RandomState so we DO NOT touch the global
                # numpy.random state — otherwise this seed (3) would deterministically
                # rewind every block's global RNG and could collide with any other
                # caller relying on global numpy.random within the experiment loop.
                # Keep the legacy MT19937 generator (RandomState) so the historic
                # outcome (severities 4 & 3, allocations 3 & 6) is preserved
                # bit-for-bit; the new-style default_rng (PCG64) would yield
                # different draws under the same seed.
                _init_rng = numpy.random.RandomState( 3 )
                # NOTE: With INIT_NO_OF_CITIES of 2, this seed always results in
                ...
                for c in range( INIT_NO_OF_CITIES ):
                    ...
                    init_severity.append( int(_init_rng.randint( MIN_INIT_SEVERITY,  1 + MAX_INIT_SEVERITY  )) )
                    ResourceAllocationsAtCurrentlyVisibleCities.append( int(_init_rng.randint( MIN_INIT_RESOURCES, 1 + MAX_INIT_RESOURCES )) )
                ...
                for trial_no in range( int( NumTrials__blocks_x_sequences__2darray[ CurrentBlockIndex, CurrentSequenceIndex ] ) ):
                    ...
                    ResourceAllocationsAtCurrentlyVisibleCities.append( -1 )   # NOTE: what does this do?
