...
def main():
    ...
    # --- Reproducibility check for this package's local artifacts ---
    ...
    confidence_exploration_strength = float(hp.get('confidence_exploration_strength', 0.0))
    confidence_exploration_exponent = float(hp.get('confidence_exploration_exponent', 1.0))
    ...
    info(f"confidence exploration strength: {confidence_exploration_strength}")
    info(f"confidence exploration exponent: {confidence_exploration_exponent}")
    ...
    # produced the mirrored q.npy (the source JSON may come from a previous
    ...
    mirrored_bp = {
        ...
        'source':            f"train_rl mirror @ {train_date} (source: {bp_payload['source']})",
    }
