...
def confidence_from_q_values(options, resources_left):
    '''Return entropy-based confidence for feasible Q-values.
    ...
    resources_left : int
        Number of resources remaining, defining the feasible actions.

    Returns
    -------
    float
        Confidence in ``[0.0, 1.0]``. A single feasible action has confidence
        ``1.0`` and tied feasible Q-values have confidence ``0.0``.
    '''
    ...
    min_entropy_distribution[0] = 1.0
