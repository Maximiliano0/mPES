...
def _save_report(study, opt_dir, opt_date, best_Q, best_rewards):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 80 + "\n")
        f.write(f"{'#':>4s}  {'mean_perf':>10s}  {'lr':>10s}  {'gamma':>8s}  {'eps0':>6s}  {'eps_min':>7s}  {'episodes':>8s}  {'warmup':>6s}  {'target':>6s}  {'beta':>8s}  {'conf_str':>8s}  {'conf_exp':>8s}\n")
        f.write("-" * 125 + "\n")
        ...
        for t in sorted(completed_trials,
            ...
            f.write(
                ...
                f"{p.get('confidence_exploration_strength', 0.0):8.4f}  "
                f"{p.get('confidence_exploration_exponent', 1.0):8.4f}\n"
            )
...
def main():
    ...
    if _best_artifacts['Q'] is not None and _best_artifacts['value'] >= best.value:
        ...
        best_rewards, best_Q, _ = QLearning(
            ...
            track_confidence=False,
            confidence_exploration_strength=bp.get('confidence_exploration_strength', 0.0),
            confidence_exploration_exponent=bp.get('confidence_exploration_exponent', 1.0),
        )
