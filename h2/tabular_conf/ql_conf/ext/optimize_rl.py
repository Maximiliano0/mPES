...
num_episodes, confidence_exploration_strength and confidence_exploration_exponent
...
Search space:
    ...
    confidence_exploration_strength ∈ [0.00, 0.60]
    confidence_exploration_exponent ∈ [0.50, 3.00]
...
# A fixed subset keeps trial scores comparable while halving evaluation I/O.
...
def objective(trial: optuna.Trial) -> float:
    ...
    ``epsilon_min``, ``num_episodes``, ``confidence_exploration_strength``
    and ``confidence_exploration_exponent``, trains a Q-table using the per-trial seed
    ``SEED + trial.number + 1``, evaluates the greedy policy with
    infeasible-action masking on a fixed 32-sequence subset, sanitises
    ...
    trial : optuna.Trial
        ...
        intermediate-value reporting (used by ``HyperbandPruner``).
    ...
    ------
    optuna.TrialPruned
        Raised when ``HyperbandPruner`` decides to abort the trial via
    ...
    # Optuna intermediate-value reporting for HyperbandPruner: receives
...
def _save_report(study, opt_dir, opt_date, best_Q, best_rewards):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write(
            ...
            f"{'episodes':>8s}  {'conf str':>8s}  {'conf exp':>8s}\n"
        )
        f.write("-" * 80 + "\n")
        for t in sorted(study.trials, key=lambda t: t.value if t.value is not None else -1, reverse=True):
            ...
            f.write(
                ...
                f"{p['num_episodes']:8d}  {p['confidence_exploration_strength']:8.4f}  "
                f"{p['confidence_exploration_exponent']:8.4f}\n"
            )
...
def main():
    ...
    (seeded with ``SEED``) and a ``HyperbandPruner``, and finally writes
    ...
    list_item("confidence_exploration_strength ∈ [0.00, 0.60]")
    list_item("confidence_exploration_exponent ∈ [0.50, 3.00]")
    print()
    ...
    if _best_artifacts['Q'] is not None and _best_artifacts['value'] >= best.value:
        ...
        best_rewards, best_Q, _ = QLearning(
            ...
            seed=replay_seed,
            track_confidence=False,
            confidence_exploration_strength=bp['confidence_exploration_strength'],
            confidence_exploration_exponent=bp['confidence_exploration_exponent'],
        )
