...
def provide_trf_agent_response(
    ...
    Notes
    -----
    - Requires TRF model pre-training via: python3 -m pes_trf.ext.train_transformer
    ...
    if not os.path.isfile(model_file):
        raise FileNotFoundError(
            ...
            f"Please train the TRF-Agent first by running: python3 -m pes_trf.ext.train_transformer\n"
        )
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Error: {str(e)}\n"
            f"Files may be corrupted. Please retrain by running: python3 -m pes_trf.ext.train_transformer\n"
        ) from e
