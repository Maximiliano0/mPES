"""
Package initialization module for pes_a2c (Pandemic Experiment Scenario
with Advantage Actor-Critic).

Variant of pes_base that replaces tabular Q-Learning with an Advantage Actor-Critic
(A2C) agent using separate Actor (policy) and Critic (value) neural networks.
The Actor produces an explicit probability distribution π(a|s) over actions, while
the Critic estimates state values V(s).  This architecture enables a theoretically
grounded confidence metric based on the entropy of π(a|s).
...
# GPU policy: by default we pin TF to CPU (the local Windows + Linux dev
# machines have no CUDA).  On Colab Pro+ with a GPU runtime the launcher
# script sets ``MPES_USE_GPU=1``; we then leave CUDA_VISIBLE_DEVICES alone
# so TF can see the device.  In either case TF is configured for
# *deterministic* ops so that the same seed reproduces the same trajectory
# (within FP tolerance — bit-exact reproduction also requires identical
# hardware and TF version on both ends).
