"""
Package initialization module for pes_rdqn (Pandemic Experiment Scenario
with Recurrent Deep Q-Network).

Variant of pes_ql that replaces the tabular Q-table with a neural-network
function approximator (RDQN) featuring experience replay and a target
network.  This __init__ is structurally identical to pes_ql/__init__;
the RDQN-specific behaviour lives in ext/rdqn_model.py, ext/train_rdqn.py,
and ext/optimize_rdqn.py.
