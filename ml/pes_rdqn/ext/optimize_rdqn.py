'''
pes_rdqn - Pandemic Experiment Scenario
...
suppressed before argmax) so that the metric matches the behaviour of the RDQN agent
...
Usage:
    python3 -m ml.pes_rdqn.ext.optimize_rdqn [n_trials] [--resume YYYY-MM-DD]
...
at the FULL ``RDQN_EPISODES`` count (default 175 000) inside the optimisation
script itself, before saving ``rdqn_best_<date>.keras``.
...
##  Imports internos    ##
##########################
from .pandemic import Pandemic, run_experiment, RDQNTraining, rdqn_agent_meta_cognitive
from .rdqn_model import build_q_network, normalize_state, HistoryDeque
from ..src.terminal_utils import header, section, success, info, list_item
from .tools import convert_globalseq_to_seqs
from ..config.CONFIG import (SEED, RDQN_LEARNING_RATE, RDQN_DISCOUNT,
                             RDQN_EPSILON_INITIAL, RDQN_EPSILON_MIN,
                             RDQN_EPISODES, RDQN_HIDDEN_UNITS, RDQN_BATCH_SIZE,
...
# is launched in isolation (e.g. ``python ext/optimize_rdqn.py``) without the
...
def objective(trial: optuna.Trial) -> float:
    """
    Optuna objective: train RDQN with sampled hyperparameters,
    evaluate on the fixed 64 sequences, and return mean normalised performance.
    """
    # --- Sample hyperparameters ---
    # Widened search space (post Double-RDQN + masking + learning_starts fix).
    # Rationale documented in pes_rdqn/doc/ — boundaries opened where the
    ...
    # (RDQN warm-up). Converted to absolute count below.
    ...
    # ``train_rdqn.py --from-best <date>`` (reads trial_seed user_attr below).
    trial_seed = SEED + int(trial.number) + 1

    rewards, model, _ = RDQNTraining(
    ...
    def qf(_env, state, seqid):
        if seqid != _qf_state['seqid']:
            _qf_history.reset()
            _qf_state['seqid'] = seqid
        norm_s = normalize_state(state, max_res, max_seq, max_sev)
        _qf_history.append_step(norm_s)
        window = _qf_history.current_window()
    ...
    _qf_history = HistoryDeque(history_len, 3)
    _qf_state = {'seqid': -1}
    ...
    # Persist the per-trial seed so train_rdqn.py --from-best can reproduce it
    ...
    # every trial — and any subsequent fresh call to ``train_rdqn.py`` —
...
def _save_report(study, opt_dir, opt_date, best_model, best_rewards):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("BAYESIAN OPTIMIZATION RESULTS — RDQN HYPERPARAMETERS\n")
        f.write("=" * 80 + "\n\n")
        ...
        # should always retrain at the full ``RDQN_EPISODES`` to match the saved
        # ``rdqn_best_<date>.keras`` (which is also retrained at full episodes).
        full_episodes = max(int(RDQN_EPISODES), int(bp['num_episodes']))
        f.write(f"RDQN_HIDDEN_UNITS = {hidden}\n")
        f.write(f"RDQN_LEARNING_RATE = {bp['learning_rate']}\n")
        f.write(f"RDQN_BATCH_SIZE = {bp['batch_size']}\n")
        f.write(f"RDQN_REPLAY_BUFFER_SIZE = {bp['buffer_size']}\n")
        f.write(f"RDQN_TARGET_SYNC_FREQ = {bp['target_sync_freq']}\n")
        f.write(f"RDQN_DISCOUNT = {bp['discount_factor']}\n")
        f.write(f"RDQN_EPSILON_INITIAL = {bp['epsilon_initial']}\n")
        f.write(f"RDQN_EPSILON_MIN = {bp['epsilon_min']}\n")
        f.write(f"RDQN_EPISODES = {full_episodes}  # full retrain length\n")
        f.write(f"RDQN_MAX_GRAD_NORM = {bp.get('max_grad_norm', 1.0)}\n")
        f.write(f"RDQN_PENALTY_COEFF = {penalty}\n")
        f.write(f"RDQN_WARMUP_RATIO = {bp.get('warmup_ratio', 0.05)}\n")
        f.write(f"RDQN_TARGET_RATIO = {bp.get('target_ratio', 0.60)}\n")
        f.write(f"RDQN_LEARNING_STARTS_FRAC = {bp.get('learning_starts_frac', 0.1)}\n")
        f.write("\n")
    ...
    ax.set_title('Bayesian Optimisation: Convergence (RDQN)', fontsize=14, fontweight='bold', pad=20)
    ...
    try:
        ...
        ax.set_title('RDQN Hyperparameter Importance', fontsize=14, fontweight='bold', pad=20)
    ...
    best_model.save(os.path.join(opt_dir, f'rdqn_best_{opt_date}.keras'))
    numpy.save(os.path.join(opt_dir, f'rewards_best_{opt_date}.npy'), best_rewards)
    success(f"Best model saved: rdqn_best_{opt_date}.keras")
    ...
    # --- Mirror to the standard input path so train_rdqn.py picks it up
...
def main():
    """Run Bayesian optimisation of RDQN hyperparameters using Optuna."""

    header("BAYESIAN OPTIMISATION — RDQN HYPERPARAMETERS", width=80)
    ...
    info("Final winner is retrained at full RDQN_EPISODES after study.optimize().")
    print()
    ...
    study = optuna.create_study(
        ...
        # Aggressive pruning: kicks in after 5 completed trials and reports
        # at every interval step, so bad trials die early and free GPU time.
    ...
    if len(study.trials) == 0:
        # num_episodes during opt is capped at 60k; the final winner will be
        # retrained at full RDQN_EPISODES afterwards.
        ...
        study.enqueue_trial({
            ...
            'use_pbrs': bool(RDQN_PENALTY_COEFF > 0),
    ...
    section("Best RDQN Model", width=80)
    ...
    # ``num_episodes >= RDQN_EPISODES`` (175 000), which the search space caps
    ...
    # model, use ``train_rdqn.py --from-best <date>`` on the local PC.
    if _best_artifacts['weights'] is not None and _best_artifacts['value'] >= best.value:
        ...
        best_rewards_list, best_model, _ = RDQNTraining(
            env_final,
            bp['learning_rate'],
            bp['discount_factor'],
            bp['epsilon_initial'],
            ...
            penalty_coeff=bp.get('penalty_coeff', RDQN_PENALTY_COEFF if bp.get('use_pbrs', True) else 0.0),
            compute_confidence=False,
