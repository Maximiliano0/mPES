"""
pes_trf - Pandemic Experiment Scenario: TRF-Agent Training Pipeline

Trains a Transformer Deep Q-Network agent on the Pandemic environment using
...
::
    ...
    python -m ml.pes_trf.ext.train_transformer [num_episodes]
...
##  Imports internos    ##
##########################
from .pandemic import Pandemic, trf_agent_meta_cognitive, run_experiment, TRFTraining
from .transformer_model import normalize_state, HistoryDeque
...
from ..config.CONFIG import (SEED, TRF_MODEL_FILE)
...
# Device selection is centralised in pes_trf/__init__.py via MPES_USE_GPU.
# Re-asserting it here only as a defensive fallback when this module is
# launched in isolation (e.g. ``python ext/train_transformer.py``) without the
...
def _load_canonical_params():
    """Load best-trial params from the canonical ``pes_trf/inputs/best_params.json``.

    This file is written automatically by ``optimize_tr.main`` as a mirror of
    ...
    # if the run was cut short, which is fine — train_trf retrains from scratch).
    model_path = os.path.join(
        INPUTS_PATH, f'{opt_date}_BAYESIAN_OPT', f'trf_best_{opt_date}.keras'
    )
...
def _load_best_trial(opt_date):
    ...
    Resolution order (mirrors pes_ql/pes_dql for cross-package consistency):
      ...
      1. ``pes_trf/inputs/<opt_date>_BAYESIAN_OPT/best_params_<opt_date>.json``
         (small JSON sidecar written by ``optimize_tr.main``).
      2. ``pes_trf/inputs/<opt_date>_BAYESIAN_OPT/optuna_study_<opt_date>.db``
    ...
    model_path = os.path.join(opt_dir, f'trf_best_{opt_date}.keras')
    if not os.path.isfile(model_path):
        model_path = None
    ...
    if trial_seed is None:
        ...
        # the per-trial seed formula in optimize_tr.objective.
...
def main():
    """Run the full TRF-Agent training and evaluation pipeline.

    Trains the TRF agent from scratch using the hyperparameters defined in
    ``config/CONFIG.py``.  Usage: ``python -m ml.pes_trf.ext.train_transformer [episodes]``.
    """

    # ---- Parse CLI arguments ------------------------------------------------
    cli_num_episodes, from_best_date = _parse_cli_args()

    header("TRF-AGENT TRAINING PIPELINE", width=80)
    ...
    train_dir = os.path.join(INPUTS_PATH, f'{train_date}_TRF_TRAIN')
    ...
    from ..config.CONFIG import (TRF_LEARNING_RATE, TRF_DISCOUNT,
                                 ...
                                 TRF_LEARNING_STARTS_FRAC, TRF_EPISODES,
                                 TRF_HISTORY_LEN, TRF_D_MODEL, TRF_NUM_HEADS,
                                 TRF_KEY_DIM, TRF_FF_DIM, TRF_NUM_LAYERS,
                                 TRF_DROPOUT)

    best_info = None
    if from_best_date is None:
        # Resolution order (highest priority first):
        #   1. pes_trf/inputs/best_params.json  (canonical mirror, always up-to-date)
    ...
    if best_info is not None:
        ...
        max_grad_norm    = bp.get('max_grad_norm', TRF_MAX_GRAD_NORM)
        ...
        if bp.get('use_pbrs', bp.get('penalty_coeff', 0.0) > 0):
            penalty_coeff = float(bp.get('penalty_coeff', TRF_PENALTY_COEFF))
        else:
            penalty_coeff = 0.0
        warmup_ratio        = bp.get('warmup_ratio',  TRF_WARMUP_RATIO)
        target_ratio        = bp.get('target_ratio',  TRF_TARGET_RATIO)
        learning_starts_frac = bp.get('learning_starts_frac', TRF_LEARNING_STARTS_FRAC)
        ...
        if num_episodes != opt_episodes:
            info(f"  Training with {num_episodes:,} episodes (CONFIG.TRF_EPISODES) "
        ...
        learning_starts_frac = TRF_LEARNING_STARTS_FRAC
        # Match the formula used in optimize_tr.objective() exactly.
        learning_starts  = max(int(learning_starts_frac * buffer_size), int(batch_size))
        num_episodes     = cli_num_episodes if cli_num_episodes is not None else TRF_EPISODES

    info(f"Starting TRF training ({num_episodes:,} episodes)...")
    info(f"Network: Input({TRF_HISTORY_LEN}, 3) -> Masking -> Dense({TRF_D_MODEL}) + PosEmb -> "
         f"[CausalMHSA(h={TRF_NUM_HEADS}, k={TRF_KEY_DIM}) + FFN({TRF_FF_DIM})] x {TRF_NUM_LAYERS} -> "
         f"last-token -> {' -> '.join(f'Dense({u}, ReLU)' for u in hidden_units)} -> Dense(11, linear)")
    info(f"Replay buffer: {buffer_size:,}  |  Batch size: {batch_size}  |  Target sync: {target_sync_freq:,}")
    info(f"Seed:          {train_seed}")
    info("(This may take several minutes)")
    print()

    rewards, trf_model, confsrl = TRFTraining(
        ...
        history_len=TRF_HISTORY_LEN, d_model=TRF_D_MODEL,
        num_heads=TRF_NUM_HEADS, key_dim=TRF_KEY_DIM,
        ff_dim=TRF_FF_DIM, num_layers=TRF_NUM_LAYERS,
        dropout=TRF_DROPOUT)
    print()
    success("Training completed")
    list_item(f"Network parameters: {trf_model.count_params():,}")
    list_item(f"Rewards history length: {len(rewards)}")

    info("Saving trained model...")

    # Save TRF model and rewards with date stamp
    model_file = os.path.join(train_dir, f'trf_model_{train_date}.keras')
    ...
    with open(config_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 80 + "\n")
        f.write(f"Network Parameters:          {trf_model.count_params():,}\n")
        ...
        f.write("-" * 80 + "\n")
        f.write(f"Model File:                  trf_model_{train_date}.keras\n")
        f.write(f"Rewards File:                rewards_{train_date}.npy\n")
        f.write(f"Configuration File:          training_config_{train_date}.txt\n\n")

        f.write("DESCRIPTION\n")
        f.write("-" * 80 + "\n")
        f.write("Files saved from TRF training on the Pandemic Scenario.\n")
    ...
    success(f"✓ Model saved to trf_model_{train_date}.keras")
    ...
    if rewards:
        ...
        ax.set_title('TRF Training: Average Reward Progression', fontsize=14, fontweight='bold', pad=20)
        ax.grid(True, alpha=0.3)
        ax.legend(loc='best', fontsize=11)
        plt.tight_layout()
        plt.savefig(os.path.join(train_dir, f'trf_agent_rewards_vs_episodes_{train_date}.png'), dpi=150,
                    bbox_inches='tight')
        plt.close()
        list_item(f"Saved: trf_agent_rewards_vs_episodes_{train_date}.png")
    else:
        info("No rewards history available — skipping reward plot")
    print()

    if True:  # pylint: disable=using-constant-test
        section("TRF-Agent Evaluation", width=80)
        ...
        # ── Parity evaluation: matches optimize_tr.objective.qf exactly ──
        ...
        _parity_history = HistoryDeque(TRF_HISTORY_LEN, 3)
        ...
        def parity_qf(_env, state, seqid):
            ...
            q_vals = trf_model(window[numpy.newaxis], training=False).numpy()[0].copy()
        ...
        _eval_history = HistoryDeque(TRF_HISTORY_LEN, 3)
        _eval_state = {'seqid': -1}
        ...
        def eval_qf(_env, state, seqid):
            ...
            _response, confidence, _rt_hold, _rt_release = trf_agent_meta_cognitive(
                q_vals, state[0], 10000
            )
        ...
        ax.plot(seqs, color='#d62728', linewidth=2.5, marker='o', markersize=5, label='TRF-Agent')
        ax.set_xlabel('Trial', fontsize=12, fontweight='bold')
        ax.set_ylabel('Final Severity Achieved', fontsize=12, fontweight='bold')
        ax.set_title('TRF-Agent Evaluation: Severity per Sequence', fontsize=14, fontweight='bold', pad=20)
        ax.grid(True, alpha=0.3)
        ax.legend(loc='best', fontsize=11)
        plt.tight_layout()
        plt.savefig(
            os.path.join(
                train_dir,
                f'trf_agent_sequence_performance_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
        list_item(f"Saved: trf_agent_sequence_performance_{train_date}.png")

        _fig, ax = plt.subplots(figsize=(12, 6))
        ax.plot(perfs, color='#9467bd', linewidth=2.5, marker='s', markersize=5, label='TRF-Agent')
        ax.set_ylabel('Normalised Performance (0-1)', fontsize=12, fontweight='bold')
        ax.set_xlabel('Trial', fontsize=12, fontweight='bold')
        ax.set_title('TRF-Agent Evaluation: Normalised Performance per Sequence',
        ...
        plt.savefig(
            os.path.join(
                train_dir,
                f'trf_agent_normalised_performance_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
        list_item(f"Saved: trf_agent_normalised_performance_{train_date}.png")

        cumperfs = numpy.cumsum(perfs)
        Domain = numpy.arange(1, 1 + 64)
        _fig, ax = plt.subplots(figsize=(12, 6))
        ax.plot(cumperfs / Domain, color='#8c564b', linewidth=2.5, marker='^', markersize=5, label='TRF-Agent')
        ax.set_ylabel('Cumulative Normalised Performance', fontsize=12, fontweight='bold')
        ax.set_xlabel('Trial', fontsize=12, fontweight='bold')
        ax.set_title('TRF-Agent Evaluation: Cumulative Performance Trend',
                      fontsize=14, fontweight='bold', pad=20)
        ...
        plt.savefig(
            os.path.join(
                train_dir,
                f'trf_agent_cumulative_performance_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
        list_item(f"Saved: trf_agent_cumulative_performance_{train_date}.png")
        ...
        ax.scatter(
            ...
            edgecolors='navy',
            linewidth=0.5)
        ax.set_title('TRF-Agent: Decision Confidence Scores During Evaluation',
                      fontsize=14, fontweight='bold', pad=20)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(train_dir, f'trf_agent_confidences_{train_date}.png'), dpi=150, bbox_inches='tight')
        plt.close()
        list_item(f"Saved: trf_agent_confidences_{train_date}.png")
        ...
        ax.set_xlabel('Trial Number', fontsize=12, fontweight='bold')
        ax.set_title('TRF-Agent: Normalised Confidence Scores', fontsize=14, fontweight='bold', pad=20)
        ax.set_ylim(-0.1, 1.1)
        ax.set_xlim(-10, 360)
        ax.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        plt.savefig(
            os.path.join(
                train_dir,
                f'trf_agent_remapped_confidences_{train_date}.png'),
            dpi=150,
            bbox_inches='tight')
        plt.close()
    ...
    success("TRF-Agent training pipeline finished successfully!")
    info(f"Output directory: {train_dir}")
    print()
#
# END OF 'main()
