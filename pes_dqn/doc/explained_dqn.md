...
# Deep Q-Network (DQN) — Teoría, Arquitectura e Implementación

> Documento técnico del paquete **`pes_dqn`** del proyecto mPES.
...
3. [¿Por qué DQN en lugar de Q-Learning Tabular?](#3-por-qué-dqn-en-lugar-de-q-learning-tabular)
4. [Arquitectura de la Red Q](#4-arquitectura-de-la-red-q)
5. [Teoría del DQN](#5-teoría-del-dqn)
...
**Deep Q-Network (DQN)** (Mnih et al., 2015) es el primer algoritmo de
...
En el contexto del proyecto **mPES**, `pes_dqn` toma la misma dinámica
...
| Archivo | Descripción |
|---------|-------------|
| `config/CONFIG.py` | Hiperparámetros DQN (red, replay, target sync, ε) |
| `ext/dqn_model.py` | Red Q, normalización, ReplayBuffer, `train_step_dqn` |
| `ext/pandemic.py` | Entorno Gymnasium, `DQNTraining`, `run_experiment` |
| `ext/train_dqn.py` | Pipeline de entrenamiento + evaluación |
| `ext/optimize_dqn.py` | Optimización Bayesiana (Optuna) |
...
| **Descuento** $\gamma$ | $0.9634$ (por defecto, mejor trial) | `CONFIG.DQN_DISCOUNT` |

### Normalización del estado

A diferencia del Q-Learning tabular (que usa índices enteros), DQN requiere
...
## 3. ¿Por qué DQN en lugar de Q-Learning Tabular?
...
- **Extensibilidad**: si se ampliase el espacio de estados (más ciudades,
  más dimensiones), DQN escalaría sin cambios.
...
llamadas a `DQNTraining` con el mismo `SEED`.
...
Con la configuración por defecto `DQN_HIDDEN_UNITS = [64, 64]`
...
DQN aproxima $Q^*(s, a) \approx Q_\theta(s, a)$ con una red neuronal
parametrizada por $\theta$.

### 5.2 TD Targets, Target Network y Double DQN

El **target** para la actualización usa el esquema **Double DQN**
...
**sobreestimación** del operador $\max$ del DQN vanilla, y la máscara de
factibilidad evita que la red bootstrappee desde acciones que no podrían
haberse tomado.

```python
# ext/dqn_model.py — train_step_dqn()  (Double DQN + feasibility mask)
...
DQN utiliza **Huber loss** (también llamada smooth L1 loss) en lugar de MSE:
...
```python
# ext/dqn_model.py — train_step_dqn()
...
DQN usa una política ε-greedy con **warm-up + decaimiento exponencial**
...
|-----------|-------------------|----------|
| $\varepsilon_0$ | 0.963 | `DQN_EPSILON_INITIAL` |
| $\varepsilon_{\min}$ | 0.069 | `DQN_EPSILON_MIN` |
| $r_{\text{warm}}$ | 0.278 | `DQN_WARMUP_RATIO` |
| $r_{\text{target}}$ | 0.629 | `DQN_TARGET_RATIO` |
| $N$ | 175 000 | `DQN_EPISODES` |
...
```python
# ext/pandemic.py — DQNTraining()  (paso de selección de acción)
...
|---------|---------|---------|
| Exponencial con warm-up (DQN) | $\varepsilon_t = \max(\varepsilon_{\min},\; \varepsilon_0 \cdot \lambda^{t - W})$ | `pes_dqn` |
...
Los tres paquetes (`pes_dql`, `pes_dqn`, `pes_a2c`) ahora comparten el
...
PARA episodio i = 1 hasta N:
    ...
    MIENTRAS no terminado:
        ...
        SI |replay_buffer| ≥ batch_size:
            ...
            train_step_dqn(online_net, target_net, optimizer, batch, γ, max_grad_norm)
...
La función `DQNTraining()` en `ext/pandemic.py` implementa el bucle
...
def DQNTraining(env, learning_rate, discount, epsilon, min_eps, episodes,
    ...
    compiled_train_step = tf.function(train_step_dqn, reduce_retracing=True)
    ...
    # 4. Warm-up del replay buffer (Double DQN sin datos suficientes es ruido)
...
`train_dqn.py` como warm-start de la próxima optimización:

| Hiperparámetro | Valor | Variable (`CONFIG.py`) |
|----------------|-------|------------------------|
| Learning rate (Adam) | $0.001508$ | `DQN_LEARNING_RATE` |
| Discount $\gamma$ | $0.9634$ | `DQN_DISCOUNT` |
| $\varepsilon_0$ | $0.9627$ | `DQN_EPSILON_INITIAL` |
| $\varepsilon_{\min}$ | $0.0691$ | `DQN_EPSILON_MIN` |
| Warmup ratio $r_{\text{warm}}$ | $0.2779$ | `DQN_WARMUP_RATIO` |
| Target ratio $r_{\text{target}}$ | $0.6290$ | `DQN_TARGET_RATIO` |
| Episodios | $175\,000$ | `DQN_EPISODES` |
| Hidden units | $[64,\; 64]$ | `DQN_HIDDEN_UNITS` |
| Batch size | $128$ | `DQN_BATCH_SIZE` |
| Replay buffer | $20\,000$ | `DQN_REPLAY_BUFFER_SIZE` |
| Target sync freq | $1\,000$ | `DQN_TARGET_SYNC_FREQ` |
| Gradient clipping | $3.953$ | `DQN_MAX_GRAD_NORM` |
| PBRS $\beta$ | $0.02258$ | `DQN_PENALTY_COEFF` |
| Learning starts (warm-up del buffer) | fracción $0.1615$ del buffer | `DQN_LEARNING_STARTS_FRAC` |
| Seed | $42$ | `SEED` |
...
- DQN tiene **un único learning rate** (para la Q-network) vs. dos en A2C
  (actor_lr + critic_lr).
- DQN incluye `batch_size`, `buffer_size` y `target_sync_freq` — conceptos
...
El callback de poda se pasa a `DQNTraining` vía `pruning_callback`:
...
```
python -m pes_dqn.ext.optimize_dqn 100 --resume 2026-03-14
```
...
(que solo carga el Actor, descartando el Critic), en DQN toda la inferencia
...
from ..ext.pandemic import Pandemic, dqn_agent_meta_cognitive
...
q_values = dqn_model(tf.expand_dims(state, axis=0),
...
La función `dqn_agent_meta_cognitive` aplica internamente el
...
> `DQNTraining` durante el entrenamiento.  Con la configuración actual
...
```python
# ext/pandemic.py — dqn_agent_meta_cognitive()
...
distribución de probabilidad genuina, los Q-values de DQN **no son
...
| **DQN** (pes_dqn) | Target network $Q_{\theta^-}$ proporciona targets estables (no elimina el sesgo, pero lo reduce al congelar $\theta^-$) |

### 11.2 Arquitectura comparada

| Componente | DQL (`pes_dql`) | DQN (`pes_dqn`) |
|------------|-----------------|-----------------|
...
- **DQN** es más **eficiente en datos** porque reutiliza transiciones del
...
| Componente | `pes_base` (Q-tabular) | `pes_dql` (Double Q) | `pes_dqn` (DQN) | `pes_a2c` (A2C) |
|------------|------------------------|----------------------|------------------|------------------|
...
`train_step_dqn` se envuelve con `tf.function` **localmente** dentro de
cada llamada a `DQNTraining`, creando un grafo JIT-compilado fresco por
trial de Optuna:
...
El cálculo de meta-cognición (`dqn_agent_meta_cognitive`) durante
entrenamiento requiere un **forward pass adicional** de la online network
(separado del forward pass de selección de acción).  Tanto `train_dqn.py`
como `optimize_dqn.py` pasan `compute_confidence=False` a `DQNTraining()`
...
Al importar `ext/dqn_model.py` se fija un único hilo intra/inter-op y se
...
bayesiana, requisito esencial para que `train_dqn.py` reconstruya con
...
La optimización bayesiana de DQN es costosa: 60 trials × 40k–100k
...
`pes_dqn/__init__.py`, `ext/train_dqn.py` y `ext/optimize_dqn.py`
...
if os.environ.get('MPES_USE_GPU', '0') != '1':
    os.environ.setdefault('CUDA_VISIBLE_DEVICES', '-1')   # pin CPU
...
| `dqn_best_<date>.keras` | Keras nativo | Modelo completo (arquitectura + pesos) |
...
Al finalizar la optimización, `_save_report()` (`optimize_dqn.py`)
emite un bloque copy-paste-ready en `optimization_results_<date>.txt`:

```python
# CONFIG.PY SNIPPET (copy-paste into pes_dqn/config/CONFIG.py)
DQN_LEARNING_RATE        = best.params['learning_rate']
DQN_DISCOUNT             = best.params['discount']
# ...
DQN_LEARNING_STARTS_FRAC = best.params.get('learning_starts_frac', 0.1)
DQN_EPISODES             = full_episodes   # NOT best.params['num_episodes']
```

La línea crítica es la última: durante la optimización,
`num_episodes ∈ [40k, 100k]` para mantener el coste bajo, pero el
reentrenamiento final usa `DQN_EPISODES = 175 000` (el valor canon de
...
`train_dqn.py` acepta una fecha YYYY-MM-DD apuntando a un directorio
...
3. Llama a `DQNTraining()` con `DQN_EPISODES` completo y
   `compute_confidence=False`.
4. Guarda el modelo final en `inputs/dqn_model.keras` (canon usado por
   `__main__.py`).

Si no se especifica `--from-best`, `train_dqn.py` busca
