# Cómo Entrenar y Testear — `pes_dqn` (Deep Q-Network)

> Guía práctica para entrenar, optimizar y ejecutar el agente DQN del
> paquete `pes_dqn` en el proyecto mPES.
...
**Variable opcional `MPES_USE_GPU`:**  Por defecto `pes_dqn` fija TF a
...
```bash
# 1. Entrenar el agente DQN (175 000 episodios por defecto)
python -m pes_dqn.ext.train_dqn
...
```bash
python -m pes_dqn.ext.train_dqn 200000
```
...
`ext/train_dqn.py` ejecuta el pipeline completo en 5 etapas:
...
### Etapa 3 — Entrenamiento DQN

Llama a `DQNTraining()` con los hiperparámetros de `CONFIG.py`
...
- Espera a que el buffer acumule
  `learning_starts = max(int(DQN_LEARNING_STARTS_FRAC · buffer_size), batch_size)`
  transiciones antes del primer paso de gradiente (warm-up del buffer);
  con los valores por defecto (`DQN_LEARNING_STARTS_FRAC ≈ 0.1615`,
  `buffer_size = 20 000`) son **3 230** transiciones
- Aplica el target **Double DQN** con enmascaramiento de acciones
...
> **Importante:** `train_dqn.py` pasa `compute_confidence=False` a
> `DQNTraining()` para reproducir exactamente los resultados de la
...
```
pes_dqn/inputs/<fecha>_DQN_TRAIN/
├── dqn_model_<fecha>.keras            # Modelo entrenado
├── rewards_<fecha>.npy                # Historial de rewards
└── training_config_<fecha>.txt        # Configuración del entrenamiento
```

Y copia el modelo a la ubicación canónica (`inputs/dqn_model.keras`)
...
| `dqn_agent_rewards_vs_episodes_*.png` | Curva de aprendizaje (reward vs episodios) |
| `dqn_agent_sequence_performance_*.png` | Severidad por secuencia (DQN) |
| `dqn_agent_normalised_performance_*.png` | Rendimiento normalizado (DQN) |
| `dqn_agent_cumulative_performance_*.png` | Rendimiento acumulado (DQN) |
| `dqn_agent_confidences_*.png` | Confianza meta-cognitiva por decisión |
...
| Discount factor (γ) | 0.9634 | `DQN_DISCOUNT` |
| Epsilon inicial (ε₀) | 0.9627 | `DQN_EPSILON_INITIAL` |
| Epsilon mínimo (ε_min) | 0.0691 | `DQN_EPSILON_MIN` |
| Warmup ratio | 0.2779 | `DQN_WARMUP_RATIO` |
| Target ratio | 0.6290 | `DQN_TARGET_RATIO` |
| Episodios | 175 000 | `DQN_EPISODES` |
| Hidden units | [64, 64] | `DQN_HIDDEN_UNITS` |
| Batch size | 128 | `DQN_BATCH_SIZE` |
| Replay buffer size | 20 000 | `DQN_REPLAY_BUFFER_SIZE` |
| Target sync frequency | 1 000 steps | `DQN_TARGET_SYNC_FREQ` |
| Max gradient norm | 3.9529 | `DQN_MAX_GRAD_NORM` |
| PBRS β (penalty_coeff) | 0.02258 | `DQN_PENALTY_COEFF` |
| Learning starts (fracción) | 0.16155 (→ 3 230 transiciones) | `DQN_LEARNING_STARTS_FRAC` |
| Random seed | 42 | `SEED` |

> Tras cada nueva optimización bayesiana, los valores óptimos se copian
> manualmente a `CONFIG.py` para que `train_dqn.py` reproduzca el mejor
> modelo encontrado.
...
automáticamente al final con `DQN_EPISODES = 175 000` para no sacrificar
calidad.
...
python -m pes_dqn.ext.optimize_dqn

# Ejecutar 100 trials
python -m pes_dqn.ext.optimize_dqn 100
...
| `num_episodes` | [40k, 100k] | paso 20k (sólo opt; el ganador se reentrena a `DQN_EPISODES`) |
...
```
pes_dqn/inputs/<fecha>_BAYESIAN_OPT/
├── dqn_best_<fecha>.keras                 # Mejor modelo
...
`train_dqn.py --from-best YYYY-MM-DD` prefiere el JSON sidecar (no requiere
Optuna instalado) y cae a la base SQLite si el JSON falta.

> **Auto-load:** Si se omite `--from-best`, `train_dqn.py` busca
...
> con `pes_ql` y `pes_dql` (basta con `python -m pes_dqn.ext.train_dqn`
...
3. Reproducir el mejor trial localmente con
   `python -m pes_dqn.ext.train_dqn --from-best YYYY-MM-DD`
   (lee `best_params_<fecha>.json` del directorio `_BAYESIAN_OPT/`).
4. (Opcional) Copiar `dqn_best_*.keras` a `inputs/dqn_model.keras`.
5. Ejecutar `python -m pes_dqn` para validar en el experimento completo.
...
El modelo entrenado debe existir en `pes_dqn/inputs/`:

```
pes_dqn/inputs/
├── dqn_model.keras      # Q-network entrenada
...
1. **Validación**: Verifica que `dqn_model.keras` y `rewards.npy` existan.
...
```
pes_dqn/outputs/<fecha>_DQN_AGENT/
...
```text
raw_mean_perf = 0.893729  (std=0.048512, n=64)
best_params.json mean_perf = 0.893729  |Δ| = 0.000000
```
...
`pes_dqn/__init__.py`, `train_dqn.py` y `optimize_dqn.py` consultan la
variable `MPES_USE_GPU` **antes** de inicializar TensorFlow:

| Valor | Comportamiento |
|-------|----------------|
| no definida o `0` | `CUDA_VISIBLE_DEVICES='-1'` (CPU forzada, 1 hilo, determinista) |
...
Al final de cada `objective()` ganador, `optimize_dqn.py` escribe a
`inputs/<DATE>_BAYESIAN_OPT/`:

| Archivo | Contenido |
|---------|-----------|
| `dqn_best_<date>.keras` | Q-network completa del mejor trial |
...
----------------------------------------------------------------
DQN_LEARNING_RATE        = 0.001508
DQN_DISCOUNT             = 0.9634
DQN_EPSILON_INITIAL      = 0.9627
DQN_EPSILON_MIN          = 0.0691
DQN_HIDDEN_UNITS         = [64, 64]
...
DQN_LEARNING_STARTS_FRAC = 0.16155
DQN_EPISODES             = 175000   # full retrain length
```

Nota: `DQN_EPISODES` se reemplaza por la longitud de **reentrenamiento
...
```text
  ...
  | optimize_dqn   |        |   OPT/<date>/     |        | train_dqn       |
...
1. **Colab**:
   ...
   ```bash
   export MPES_USE_GPU=1
   python -m pes_dqn.ext.optimize_dqn 60
   ```
...
3. **PC local**:
   ...
   ```bash
   python -m pes_dqn.ext.train_dqn --from-best <date>
   ```

   `train_dqn.py` lee `best_params_<date>.json`, fija el seed y
   reentrena con `DQN_EPISODES` completo en CPU determinista.
...
> que son exactamente los valores usados durante `DQNTraining`.  Con la
...
### No se encuentra `dqn_model.keras`

```
FileNotFoundError: dqn_model.keras not found
```

**Solución**: Entrenar primero con `python -m pes_dqn.ext.train_dqn`.
...
DQN usa `SEED = 42` para NumPy, Python `random`, TF y —adicionalmente—
para un `numpy.random.Generator` **dedicado** a la rama aleatoria de
ε-greedy.  Además, `dqn_model.py` fija un único hilo intra/inter-op y
...
> **Atención:** Si `DQNTraining()` se llama con `compute_confidence=True`
> (el default), las llamadas a `numpy.random.normal()` dentro de
> `dqn_agent_meta_cognitive` consumen ≈1.5 M de números aleatorios
...
> **Solución:** Asegurar que `train_dqn.py` pase `compute_confidence=False`.
...
| Tarea | Comando |
|-------|---------|
| Entrenar (175k episodios) | `python -m pes_dqn.ext.train_dqn` |
| Entrenar (N episodios) | `python -m pes_dqn.ext.train_dqn N` |
| Optimización Bayesiana (60 trials) | `python -m pes_dqn.ext.optimize_dqn` |
| Optimización (100 trials) | `python -m pes_dqn.ext.optimize_dqn 100` |
| Reanudar optimización | `python -m pes_dqn.ext.optimize_dqn 100 --resume YYYY-MM-DD` |
| Ejecutar experimento | `python -m pes_dqn` |
