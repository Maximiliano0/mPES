"""
pes_dqn — Configuration file.

Centralises every tuneable parameter for the Pandemic Experiment Scenario
(Deep Q-Network).  Values here apply to the experiment loop (__main__.py),
DQN training (train_dqn.py), and Bayesian optimisation (optimize_dqn.py).
...
Key Differences vs pes_dql (Double Q-Learning):
    ...
    - DQN-specific hyperparameters: hidden units, replay buffer, target sync,
    ...
    - SEED = 42 (fixed for reproducible DQN training & optimisation)
...
Sections:
    ...
    - Player & Agent Settings        (DQN_AGENT selection)
    - Pandemic Dynamics              (α / β multipliers)
    - UI & Interaction               (trust scale, fixed sequences)
    - Reproducibility                (SEED = 42)
    - DQN Hyperparameters            (network, replay buffer, target sync)
...
# ==================== OUTPUT & LOGGING ====================
OUTPUT_FILE_PREFIX = 'PES_DQN_'    # Prefix for all output filenames
...
# ==================== DQN HYPERPARAMETERS ====================
# Best hyperparameters from Bayesian Optimisation trial #41 (2026-04-23, Colab Pro CPU).
# These values serve as CONFIG defaults AND as warm-start for optimize_dqn.py.
# Performance: mean_perf = 0.893729 over 64 fixed sequences.
DQN_HIDDEN_UNITS = [64, 64]                        # Hidden-layer widths for the Q-network
DQN_LEARNING_RATE = 0.0015083436603048935          # Adam learning rate for the Q-network
DQN_BATCH_SIZE = 128                               # Mini-batch size sampled from replay buffer
DQN_REPLAY_BUFFER_SIZE = 20_000                    # Maximum transitions stored in the replay buffer
DQN_TARGET_SYNC_FREQ = 1_000                       # Steps between hard copies of online → target network
...
DQN_TARGET_RATIO = 0.6290206520891799              # Fraction at which ε reaches ε_min via exponential decay
DQN_LEARNING_STARTS_FRAC = 0.16154748671160965     # Fraction of buffer_size that must accumulate before training starts
DQN_MODEL_FILE = 'dqn_model.keras'                 # Filename for the saved Q-network weights
